...
| 60 | La factura cedida a un factoring AJENO no es candidata del inbound (cuarta condición de «Buena factura», sobre el A2); la cedida a Security sí lo es y al incorporar entra | [`reglas/prospeccion_cartera_y_churn.md`](./reglas/prospeccion_cartera_y_churn.md) | 159, `regla_60.test.mjs` |
| 61 | La factura emitida hace más de `antiguedadMaxDias` días (tenant, 20 por defecto; contra el corte del activo) no es candidata del inbound: quinta condición de «Buena factura»; «no más de N» incluye el día N; el perfil nombra «Antigüedad > N días (excluida)» y la Bandeja muestra la antigüedad del documento, no un 1 fijo | [`reglas/prospeccion_cartera_y_churn.md`](./reglas/prospeccion_cartera_y_churn.md) | 160 |
| 62 | Ninguna excepción sin justificar en la mutación de cierre: `cerrarOferta` re-comprueba las excepciones mudas (`compuertaExcepcionesMudas`) antes de escribir y devuelve la negativa; `solicitarAprobacionExc` no guarda una solicitud sin comentario, respaldo ni declaración; «Enviar de todos modos» envía con la declaración | [`reglas/curse_firma_y_etapas.md`](./reglas/curse_firma_y_etapas.md) | 161, `regla_62.test.mjs` |
| 63 | El resultado de líneas entra al giro: un deudor con facturas `REQUIERE_COMITE` califica Giro Normal aunque cumpla Express (quinto hecho `sinComite`, declarado en `GIRO_HECHOS` y exigido por GE; el adaptador lo saca de la versión); sin comité deciden los cuatro hechos y la suma por tipo sigue siendo el monto a girar | [`reglas/oferta_pricing_y_giro.md`](./reglas/oferta_pricing_y_giro.md) | 162 |
| 64 | El corte y el reinicio del día son por reloj del tenant (`horaFin` 23:00 / `horaInicio` 06:00, `jobDelReloj`; `relojSimulado` en la demo), no por conteo de corridas; al corte la oportunidad del inbound sin oferta se elimina (`corteDelDia`, con su cierre en la bitácora) y la que tiene oferta no se toca; al reinicio vuelve como oportunidad nueva con id propio (`-R<n>`) y `referencia`; `frecuenciaMin` gobierna el intervalo del job (`intervaloJobMs`); el esquema `cfgOper` v3 retira `etapaNoGestionada` y `horasDia` | [`reglas/prospeccion_cartera_y_churn.md`](./reglas/prospeccion_cartera_y_churn.md) | 163, 164, `regla_64.test.mjs` |
| 65 | El comité de crédito puede rechazar la línea puntual: la API 3 devuelve «Rechazada» por línea de detalle y, al consultarla, NEX retira de la oferta las facturas de ese deudor, deja versión con motivo `comite_rechazo` (la asignación sólo encoge) y reabre la operación revocando la firma; si no queda factura, la pierde con causa «Línea rechazada por el comité»; «Observada» y «Aprobada» no tocan nada | [`reglas/lineas_y_solicitud_comite.md`](./reglas/lineas_y_solicitud_comite.md) | 165, `regla_65.test.mjs` |
| 66 | La excepción que la versión N ya no levanta se marca «ya no aplica desde la versión N» —solicitud, visado, tarea e hilo, con actor sistema y hora; decisión pura `excepcionesQueYaNoAplican`, mutación desde `reevaluarCliente`— y nada se borra; la marca no es decisión (`excSinVisar`/`solVigente` en todo lector) y si la regla vuelve a levantar está pendiente otra vez: la marcada no justifica ni se reactiva y la solicitud nueva la lleva como historia | [`reglas/otorgamiento_y_atribucion.md`](./reglas/otorgamiento_y_atribucion.md) | 166, `regla_66.test.mjs` |
| 67 | La verificación fallida marca y avisa, no retira: `marcarNoVerificada` es el único escritor del veto (sin tocar `facturasOp`, sin versión, sin mover etapa ni firma), `verifResumenDeal`/`issueVerificacion` nombran las marcadas (VER-01 sigue mandando y lo dice), `avisarNoVerificadas` escribe al ejecutivo como el sistema; el ejecutivo retira con la operación reabierta, re-simula y vuelve a publicar para la nueva firma; `retirarFacturaOferta` sin la excepción «noConfirmada» | [`reglas/verificacion.md`](./reglas/verificacion.md) | 167, `regla_67.test.mjs` |
...
| `regla_<slug>.test.mjs` (47 archivos, 17-09-2026) | Un gate por regla de dominio que vive en JSX o en un closure y no tiene función pura que la suite pueda llamar: se fija sobre el TEXTO del fuente (el sitio, la compuerta, el rótulo, la unidad) y cada uno trae sus sondas | regla · alguno **snapshot** (`TOPE_REINTENTOS`) |
