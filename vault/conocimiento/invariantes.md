...
| 47 | El stream del inbound se reparte por ROL: el ejecutivo ve sólo su cartera, el rol `inbound` y la jefatura sólo lo que no es de nadie | [`reglas/prospeccion_cartera_y_churn.md`](./reglas/prospeccion_cartera_y_churn.md) | `regla_40.test.mjs` |
| 48 | Al cerrar el negocio el sistema le escribe a los aprobadores de las excepciones pendientes y al ejecutivo; el remitente es el sistema y calla si no queda nada que firmar | [`reglas/otorgamiento_y_atribucion.md`](./reglas/otorgamiento_y_atribucion.md) | 152, `regla_aviso_cierre.test.mjs` |
| 49 | El estado del otorgamiento (pre-evaluación, hilos, visado) cruza de pestaña con storage Y aviso al opener: el detalle es pestaña propia y un `let` de módulo no sale de su documento | [`reglas/otorgamiento_y_atribucion.md`](./reglas/otorgamiento_y_atribucion.md) | 153, `regla_estado_pestanas.test.mjs` |
| 50 | El tenant se da de alta en la plataforma (`nex_tenants`, sin sufijo) con su marca y su ADMINISTRADOR; el super-admin es un ROL y no el código «ADMIN» | [`reglas/otorgamiento_y_atribucion.md`](./reglas/otorgamiento_y_atribucion.md) | 154, `regla_tenants.test.mjs` |
