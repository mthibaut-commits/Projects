/* Gate de contrato de la regla 15-bis-bis sobre el TEXTO del fuente: la suite prueba al receptor, pero que el
   ...
   deduplica por `idProceso` y NO rearma (no llama a `api1Inyeccion` ni toca `SOLIC_SEQ`), se llame como se llame
   su parámetro. */
...
export function emisorEnCerrarOferta(src) {
  ...
  if (!/^[A-Za-z_$][\w$]*$/.test(expr)) return { ok: false, motivo: `el registro posteado es \`${expr}\`: ni SOLICITUDES_LINEA[0] ni una variable local asignada desde él` };
  ...
  if (!ES_REGISTRO_CERO.test(valor)) return { ok: false, motivo: `\`${expr} = ${valor}\`: no es SOLICITUDES_LINEA[0], el registro que api1Inyeccion acaba de dejar` };
...
export function tramoListenerTubo(src) {
  ...
  if (!L.slice(fin + 1, fin + 4).some((l) => /window\.addEventListener\("message", onMsg\)/.test(l))) return { error: "tras el cierre de onMsg no viene `window.addEventListener(\"message\", onMsg)`: el tramo no es el listener" };
...
export function receptorNoRearma(src) {
  ...
  const p = m[1], cuerpo = m[2], P = p.replace(/\$/g, "\\$");
  if (!new RegExp(`\\b${P}\\.idProceso`).test(cuerpo)) return { ok: false, motivo: `el receptor no mira ${p}.idProceso` };
  if (!new RegExp(`\\.some\\(.*idProceso === ${P}\\.idProceso`).test(cuerpo)) return { ok: false, motivo: "el receptor no deduplica por idProceso" };
  if (/api1Inyeccion\(|SOLIC_SEQ/.test(cuerpo)) return { ok: false, motivo: "el receptor REARMA la solicitud (api1Inyeccion/SOLIC_SEQ): daría otro id" };
...
test("15-bis-bis · cerrarOferta postea `nex-solicitud` con el registro recién inyectado (SOLICITUDES_LINEA[0])", () => {
  ...
  assert.equal(emisorEnCerrarOferta(enTramo(sustituir(t, "registro: SOLICITUDES_LINEA[0]", "registro: idProc"))).ok, false, "sonda: postear el id en vez del registro tiene que fallar");
  assert.equal(emisorEnCerrarOferta(enTramo(sustituir(t, "registro: SOLICITUDES_LINEA[0]", "registro: sol"))).ok, false, "sonda: postear los datos de entrada (rearmables) en vez del registro tiene que fallar");
  // Refactor CONFORME: el registro va por una variable local asignada desde SOLICITUDES_LINEA[0] tras inyectar…
  const conVar = sustituir(sustituir(t, LINEA_INY, LINEA_INY + "\n          const registroNuevo = SOLICITUDES_LINEA[0];"), "registro: SOLICITUDES_LINEA[0]", "registro: registroNuevo");
  ...
  const varAntes = sustituir(sustituir(t, LINEA_INY, "const registroNuevo = SOLICITUDES_LINEA[0];\n          " + LINEA_INY), "registro: SOLICITUDES_LINEA[0]", "registro: registroNuevo");
  ...
  const varOtra = sustituir(sustituir(t, LINEA_INY, LINEA_INY + "\n          const registroNuevo = sol;"), "registro: SOLICITUDES_LINEA[0]", "registro: registroNuevo");
...
test("15-bis-bis · el listener del tubo (onMsg de PipelineComercial) rutea `nex-solicitud` a recibirSolicitudLinea(m.registro)", () => {
  ...
  assert.equal(listenerRutea(sustituir(jsx, "recibirSolicitudLinea(m.registro)", "api1Inyeccion(m.registro)")).ok, false, "sonda: rearmar con api1Inyeccion en el listener tiene que fallar");
  ...
  assert.equal(listenerRutea(sustituir(jsx, "export default function PipelineComercial(", "export default function PipelineComercialX(")).ok, false, "sonda: sin PipelineComercial no hay listener del tubo que gatear");
});

test("15-bis-bis · recibirSolicitudLinea es de nivel módulo, deduplica por idProceso y no rearma", () => {
  ...
  const sinDedup = sustituir(jsx, "  if (SOLICITUDES_LINEA.some((s) => s && s.idProceso === reg.idProceso)) return false; // idempotente\n", "");
  assert.equal(receptorNoRearma(sinDedup).ok, false, "sonda: sin la deduplicación tiene que fallar");
  const anidado = sustituir(jsx, "\nfunction recibirSolicitudLinea(reg) {", "\nconst _envoltura = () => {\nfunction recibirSolicitudLinea(reg) {").replace("  SOLICITUDES_LINEA.unshift(reg);\n  return true;\n}", "  SOLICITUDES_LINEA.unshift(reg);\n  return true;\n}\n};");
  assert.equal(receptorNoRearma(anidado.replace(/^function recibirSolicitudLinea/m, "  function recibirSolicitudLinea")).ok, false, "sonda: un receptor que no es de nivel módulo tiene que fallar");
