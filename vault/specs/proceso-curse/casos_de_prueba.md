...
- **Criterio**: CA-5 de HU-01 · **Dirección**: positiva (la NC parcha los disponibles y la oferta abierta, con traza) y negativa (sobre la oferta cerrada el documento queda con su estado y marcado `inhabilitada`, con traza en rojo; la re-entrega no se aplica dos veces; un folio ajeno no toca nada) · **Capa**: suite. · **Cobertura actual**: **caso 170** (23-09-2026, ADR-0020, regla 70; el paso 4 fija desde ADR-0021 la inhabilitación) y `regla_70.test.mjs` (el tick separa las actualizaciones antes de clasificar; el pliegue del fuente ≡ el del generador).
...
- **Criterio**: CA-5 de HU-01 (la mitad de la regla 71) · **Dirección**: negativa (no se cursa: el veto cuenta como pendiente aunque la llamada esté registrada; VER-01 lo dice; la candidata bloquea) y positiva (el issue y el aviso nombran folio y motivo; el acuse no inhabilita; sólo con el veto de la llamada nada cambia respecto del CP-138) · **Capa**: suite. · **Cobertura actual**: **caso 171** (23-09-2026, ADR-0021, regla 71) y `regla_71.test.mjs` (el tick escribe el veto por el único escritor y fuera del updater; el escritor firma como el SII; la tarjeta, la fila y el oyente de storage).
...
- **Criterio**: CA-1 de HU-21 (en pantalla; M-36, decisión del 22-09-2026: «guarda en la versión el tipo de modelo que se utilizó para simular (tasa ponderada o última operación) y las condiciones de descuento y comisiones que se asignaron») · **Dirección**: positiva (la versión lo trae y cambia con el modo) y negativa (sin cambiar el modo, dos versiones seguidas lo repiten igual) · **Capa**: e2e + suite. · **Cobertura actual**: **cubierto el 23-09-2026** por el caso **168** (`CFG_ACTIVA.tasaModo` en «riesgo» y en «ultima» deja dos versiones que difieren en `pricing.modo` y no en la huella) y por **`e2e-68-d`** (la pantalla: gear → «Selección de la tasa del negocio» → un detalle nuevo → re-evaluar → `SIM_VERSIONS`).
...
- **Escrito el 23-09-2026**: `e2e-68-d` (gear → `select` con la opción `riesgo` → otro modo; la config persiste en el acto y un detalle NUEVO la lee; «Re-evaluar simulación» —o una factura más y «Re-evaluar operación» si no hay re-evaluables— deja la versión n+1 con el modo nuevo, la n conserva el suyo y el paquete no se mueve; `finally` devuelve el modo).
...
3. **Luego, los T2 decididos y los que no esperan decisión**: CP-064/067 (G-12: implementados, caso 161), CP-012/120 (G-31: implementados, caso 160), CP-019/020/121 (G-02, G-03: implementados, casos 163–164),
   CP-010 y CP-011 (G-01: **hechos el 23-09-2026**, regla 69, caso 169, `regla_69` y `e2e-69-a`), CP-005/006 (G-29), CP-013 (control de configuración), CP-042
