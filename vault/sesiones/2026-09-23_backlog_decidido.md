...
## 1 · ADR-0014 · La cedida a un factoring ajeno no es candidata (regla 63, caso 159)
...
rotulan «Cedida a Security» (el `SHORT_EST` del candado perdió sus dos claves de cesión propia) y la regla 63 quedó
con gate de texto propio, `regla_63.test.mjs`, que fija las tres piezas con sonda cada una: la cuarta condición, la
...
## 2 · M-10 · La antigüedad máxima desde la emisión, configurable (regla 64, caso 160)
...
HU-05 vigente en CA-1/CA-2, CP-012/120 con el caso 160 (66 cubiertos · 67 nuevos · 100 CP piden caso), regla 64 y su
fila, cifras (160/160; 90 reglas).

## 3 · M-19 · Ninguna excepción sin justificar en la mutación de cierre (regla 65, caso 161)
...
caso 85) y el gate de texto `regla_65.test.mjs` fija que la llama y retorna antes de escribir. Es el mismo reparto que
...
(el chip «Operación creada» de CA-2 sigue por e2e, CP-065), CP-064/065/067 (caso 161), regla 65 y su fila, cifras.

## 4 · ADR-0017 · El resultado de líneas entra al giro: con facturas a comité, Giro Normal (regla 66, caso 162)
...
gaps (G-20 y G-34 cerrados), HU-37 vigente, CP-104/128 (caso 162; el chip en pantalla sigue por e2e), regla 66 y la 22
ampliada, cifras.

## 5 · ADR-0019 · El corte y el reinicio por reloj del tenant; la sin oferta se elimina (regla 67, casos 163–164)
...
- **La regla 64 había quedado intercalada** entre las viñetas de la 60 (M-10 la insertó tras la viñeta equivocada); se
...
CP-019/020/121/022/023/143 (casos 163–164), regla 67 y punteros en 22 y 9-bis, cifras.

## 6 · ADR-0015 · El comité que rechaza retira, versiona y reabre (regla 68, caso 165)
...
**Documentos:** regla 68, punteros en 13, 15 y 5, contrato de la API 3 (Integraciones y swagger), spec del curse (M-29;
...
## 7 · ADR-0016 · La excepción que la versión N ya no levanta se marca, no se borra (regla 69, caso 166)
...
**Documentos:** regla 69 (puntero en la 4), fila en `invariantes.md`, spec del curse (M-21 implementada: 33 · 6 · 1; §15),
...
## 8 · ADR-0018 · La verificación fallida marca y avisa, no retira (regla 70, caso 167)
...
**Documentos:** regla 70 (punteros en la 6 y en la 13), fila en `invariantes.md`, spec del curse (M-18 implementada
entera: 34 · 6 · 0; §15 dos filas), gaps (G-11 en M-18 y G-36 cerrados; 13 implementados · 5 decididos), HU-42 vigente y
HU-33 CA-3 en su dirección nueva (28 · 14), CP-091/119/129/138–142 (caso 167 y `regla_70`; la pantalla sigue por e2e),
`spec-verificacion-facturas.md` §1 y §9, `spec-ciclo-factura.md` §14, cifras (167/167; 96 reglas; 57 archivos de gate, 47
por regla; 486 tests).

## 9 · ADR-0013 · Un evento de evaluación, cinco motores, cinco versiones (regla 71, caso 168)
...
- **Gates re-anclados, no aflojados**: `regla_69` (la marca la dispara el evento), `regla_14` (4) (la emite
...
**Documentos:** regla 71 (punteros en la 13 y la 14; la 66 y `ui_detalle_y_tubo.md` apuntan al evento), fila en
...
## 10 · M-01 · El acuse del receptor es una bandera del DTE que el A1 trae y `facturaDeDTE` lee (regla 72, caso 169)
...
**Documentos:** regla 72 en `datos_y_activos.md` (fila en `invariantes.md`), `spec-inbound-facturas.md` §2 y §3,
...
## 11 · Las pantallas que los commits dejaron dichas: `28_version_v1.e2e.mjs` (e2e-71-a/b/c/d, e2e-72-a)

**Qué cambió.** Cinco casos e2e sobre la fila 0 de «Sin línea» del Directorio, encadenados y cada uno dejando el
estado que necesita si el anterior no llegó: `e2e-71-c` (sin simular: cero versiones, sin titular y sin compuertas
—el pie no existe todavía, la pantalla muestra el panel de arranque—), `e2e-71-a` (dos facturas a mano y «Simular la
oferta»: la v1/rev 0 con las cinco secciones sobre esos dos folios, contemporánea, el TUBO la ve sin recargar y la
auditoría tiene el evento), `e2e-71-b` (cerrar y reabrir: misma cantidad de versiones, misma vigente, mismo titular y
mismo pie), `e2e-71-d` (gear → «Selección de la tasa del negocio» a otro modo → un detalle nuevo lo lee → «Re-evaluar
simulación» deja v2 con el modo nuevo y la v1 conserva el suyo; el `finally` devuelve el modo) y `e2e-72-a` (el chip del
...
- **Los ids de los esbozos (`e2e-13-a`, `e2e-HU-21-a`) se escribieron antes de que existieran las reglas 71 y 69**: los
...
**Documentos:** `invariantes.md` (filas 68 y 69, cifra de la capa), reglas 71 y 69, CP-034/035/036/123/010 (los
esbozos pasan a casos escritos), HU-13/HU-21/HU-04, cifras (37 casos e2e en 19 archivos), tablero.

## 12 · ADR-0020: el A1 es un flujo de eventos por documento (regla 73, caso 170)
...
- **Gates**: `regla_73.test.mjs` (14 tests: el pliegue del fuente EXTRAÍDO y ejecutado en Node contra el del generador,
...
**Documentos:** ADR-0020 (+ fila en `adr/index.md`), regla 73 en `datos_y_activos.md` (fila 70 en `invariantes.md`, y las
...
## 13 · ADR-0021: sobre la oferta cerrada, publicada o firmada la NC, el reclamo o la cesión a otro inhabilitan el documento (regla 74, caso 171)
...
- Caso 170 (d) pasó a fijar la inhabilitación; caso 171 nuevo; `regla_74.test.mjs` (17 tests) y `regla_73` re-anclado.
  `CASOS_ESPERADOS` 170 → 171 (decisión). Gates 552/552 (62 archivos, 51 por regla).

**Lo que costó / sorpresas.**
- **Las anclas de tres gates vecinos.** `regla_63` planta su sonda sobre `const motivoExcl = (f) => {const c = cesionDeFactura(`
  (la línea nueva va DESPUÉS de la cesión); `regla_71` exige que `onStorage` empiece con su guarda original (el veto tiene
  su propio oyente, `onStorageVeto`, declarado antes); `regla_70` exige `repoNoConfirmadas.set(id, nc); const deudor = …`
...
- **`verifResumenDeal` dejaba cursar un documento vetado con la llamada en verde**: la regla 70 decía «cuentan también en
...
**Documentos:** ADR-0021 (+ índice), regla 74 en `verificacion.md` (+ lista de la cabecera), regla 73 reescrita en la
