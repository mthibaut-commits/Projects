...
- Componentes grandes: `PipelineComercial` (App), `DealDrawer` (detalle op), `DealCard`/`TaskCard`, `LineasView` (sub-tabs Vigentes/En proceso), `PresentacionComite` (wizard 6 pasos), `PanelClientes` (Sankey d3), `PlanPorEjecutivo`, `VerificacionTab`, `OtorgamientosView`.

## Reglas de dominio (invariantes — NO romper)

1. **Curse nunca por email:** el cliente acepta SOLO firmando en el portal (factoringsecurity.cl/curse). El ejecutivo no puede mover a Aceptada (bloqueado en drag y selector).
