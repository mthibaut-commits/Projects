/* Gate de contrato de la regla 62 (ninguna excepción sin justificar en la MUTACIÓN de cierre; solicitar sin
...
export function auditarRegla62(src) {
  const fallos = [];
...
test("regla 62: la mutación de cierre rechaza las excepciones sin justificar, solicitar sin justificación no escribe y la pre-evaluación declara", () => {
  assert.deepEqual(auditarRegla62(jsx), []);
});
...
for (const [nombre, mutar] of MUTANTES)
  test(`sonda negativa: ${nombre}`, () => {
    ...
    assert.ok(auditarRegla62(mut).length > 0, "el gate no cazó la violación plantada");
  });
