...
| [ADR-0020](./ADR-0020-el-a1-es-un-flujo-de-eventos-por-documento.md) | El A1 es un flujo de eventos por documento: la creación, la nota de crédito y el acuse llegan por separado; el documento se pliega en un solo sitio por lado (generador y fuente, gateados iguales); el inbound aplica cada actualización donde el documento vive y sobre la oferta cerrada sólo avisa | aceptada · modelo del usuario 23-09-2026 («los eventos de dtesync llegan varias veces para la misma factura»); queda abierta la NC o el reclamo sobre una oferta publicada o firmada (cerrada el mismo día: ADR-0021) |
| [ADR-0021](./ADR-0021-la-nc-el-reclamo-o-la-cesion-ajena-sobre-la-oferta-cerrada-inhabilitan-el-documento.md) | Sobre un documento de una oferta cerrada, publicada o firmada, la NC, el reclamo o la cesión a otro lo inhabilitan y dejan la operación no cursable: el veto es el de la regla 71 escrito por el SII, cuenta como pendiente aunque la llamada esté en verde, y el ejecutivo retira, re-evalúa y vuelve a firmar | aceptada · decisión del usuario 23-09-2026 (decisión #7 de `spec-inbound` §12) |
| [ADR-0022](./ADR-0022-las-skills-de-terceros-y-los-servidores-mcp-salen-del-repo.md) | Las skills de terceros y los servidores MCP salen del repo: no aportan al producto; se quedan la skill propia `datamart-ui` y la regla 63 con su gate de paleta | aceptada · decisión del usuario 23-09-2026 («todas estas ramas no aportan nada al proyecto ya que instalan librerías») |
| [ADR-0023](./ADR-0023-la-perdida-por-cesion-a-la-competencia-es-un-hecho-del-a2.md) | La pérdida por cesión a la competencia es un hecho del A2: la oferta se pierde entera sólo si otro factoring se llevó todas sus facturas, una parte se anota, y el sorteo del 12% se retira; `avanzarPipeline` queda (el gate de la regla 48 vigila su texto) | aceptada · hallazgo 8 de la revisión de Reportes, 23-09-2026; el criterio es el que ya tenía el código muerto; retirar el motor queda como decisión del usuario |
| [ADR-0024](./ADR-0024-la-maquina-de-estados-son-dos-tramos-y-cada-transicion-tiene-un-solo-escritor.md) | La máquina de estados son DOS TRAMOS —el comercial ordenado y el posterior a la firma sin orden interno, porque cesión y otorgamiento son atributos desacoplados— y cada transición se decide en una función pura con UN escritor: tras el otorgamiento se queda Pendiente Integración y sólo `aprobarIntegracion` escribe el giro, la pérdida exige causa y se delega en `reject`, y WhatsApp manda el enlace y no la firma | aceptada · instrucciones del usuario 23-09-2026 sobre G-24, G-25 y G-26 |
| [ADR-0025](./ADR-0025-el-nivel-de-una-excepcion-se-calcula-una-vez-y-toda-pantalla-lo-lee.md) | El nivel de una excepción se calcula UNA vez, con el monto de la operación, y toda pantalla lo lee: la versión guarda el nivel exigido (no el del tramo) y conserva el del tramo aparte; el respaldo del nivel ausente es uno solo | aceptada · reporte del usuario 24-09-2026 (Jefe de Operaciones sin sus excepciones ni sus mensajes), medido en el detalle real |
| [ADR-0026](./ADR-0026-el-corte-lee-el-estado-del-proceso.md) | El corte del día lee el estado del PROCESO, no sólo la copia de la pestaña del tubo: la versión emitida es oferta y la pre-evaluación pedida es gestión, así que la operación que el ejecutivo trabaja en el detalle no se elimina mientras el aviso viaja | aceptada · reporte del usuario 24-09-2026 (Subgerente de Riesgo sin la operación en la mesa), medido en el detalle real |
