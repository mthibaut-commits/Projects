/* Gate de contrato de la regla 60 (la factura cedida a un factoring AJENO no es candidata del inbound; la cedida a
...
export function auditarRegla60(src) {
  const fallos = [];
...
test("regla 60: la cedida a un factoring ajeno no es candidata; la cedida a Security es agregable y la oferta la cuenta", () => {
  assert.deepEqual(auditarRegla60(jsx), []);
});
...
for (const [nombre, mutar] of MUTANTES)
  test(`sonda negativa: ${nombre}`, () => {
    ...
    assert.ok(auditarRegla60(mut).length > 0, "el gate no cazó la violación plantada");
  });
