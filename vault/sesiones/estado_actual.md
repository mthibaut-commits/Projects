...
(`pipeline_comercial.jsx`), build standalone, **150/150 PASA**, **39 archivos de gate de contrato**
(265 tests), **29 casos e2e** (28 verdes), `tsc` limpio, 0 duplicados. El 17-09-2026 el repo abrió su
vault (73 reglas verbatim por tema, índice en `invariantes.md`), cableó sus gates (ADR-0001, ADR-0002) y
...
al comité** (146), las **identidades pasan a ser pares reales del AEC** (42, ADR-0009) y, al cierre, la
**estructura de líneas deja de fabricarse en el pipeline** (44, 150, ADR-0010). De `main`, los **controles
de la integración al core** (41, ADR-0007).

**La regla 44 en una línea:** el activo A7/A8 trae **466 filas** sin nivel de deudor y el motor consumía
**3.636 objetos de línea**; los 3.170 restantes los sorteaba `lineasDeCliente` partiendo del A7 — lo que
el levantamiento §5.4 prohíbe. Hoy los tres niveles llegan por el **activo A23** (`LINEA_CUPO` 4.136 filas
+ `LINEA_DEUDOR` 741), los produce `GeneradorDatos/datasets/lineas_par.js` y el pipeline sólo los lee. La
migración **no movió un peso** (snapshot previo en Chromium, port validado contra él en Node). Con eso
**se cerró el bucle del comité**: `constituirLinea` ya no descarta `sol.detalle`, así que el deudor que va
al comité por no tener LF2/LF3 vuelve con su puntual y la asignación siguiente la usa (caso 150, en las
dos direcciones). `lineaMinima` y `otrosDeudoresPct` quedaron **declarativas** y su `hint` lo dice.
...
> 3. **Correr el `.bat` una vez**: `build_app.ps1` cambió (embebe `arte_login.js`) y acá no se ejecuta; la
>    simetría está gateada, no probada.
> 4. **Regenerar `Capturas_UI/`**: la UI muestra los nombres reales del padrón, así que la fuente de Figma
>    quedó desfasada. Atado a la deuda 2.
...
1. **Auditores**: `BASE_MUERTOS` queda en **1** (`giroDeal`), candidato a poda. · **GN como disyunción**
   ...
   LineaSugeridaMM` (600 filas) es un campo heredado **sin uso, sin documentar y nombrado en millones**:
   no lo confundan con una señal de línea por deudor (ADR-0010).
...
Últimas: [las líneas son un insumo](./2026-09-20_las_lineas_son_un_insumo.md) · [el padrón real](./2026-09-20_padron_real_del_aec.md) · [la mezcla con main](./2026-09-20_mezcla_con_main.md)
