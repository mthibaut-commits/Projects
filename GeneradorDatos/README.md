...
| `PLATAFORMA360` | A11 | `datasets/plataforma360.js` | Maestro de empresa por RUT (clientes y deudores): firmográfica, comercial, socios, índices y la **nota de comportamiento**, que vive sólo acá. Razón social y ventas salen de DTESync; colocación y última operación, de AECSync; segmento, del SOW. El **mix de financiamiento** (`SOW_*`, 4 porciones que suman 100) se construye sobre el `SOWActualPct` del A5 —al que no puede contradecir: las tres porciones de factoring renormalizadas lo reproducen— y sobre el reparto medido en AECSync entre el factoring target y el resto; sólo la porción **bancaria que no es factoring** se genera por perfil, porque ningún activo la trae y ésa es justamente la razón de que el mix viva en el A11 |
