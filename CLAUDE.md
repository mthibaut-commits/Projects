...
`pipeline_comercial.jsx`, que el build embebe junto a `datos_inyectados.js` (~34 MB; **sin él el pipeline queda
...
PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers node run_tests.mjs                                                  # 5 · 161/161 PASA (~2 min)PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers node tests/e2e/correr.mjs                                           # 6 · e2e: 32 casos de pantalla (~8 min)
...
contrato: `vault/conocimiento/invariantes.md` § Gates. **Desde el 17-09-2026 ninguna regla queda «sin gate»**: las 91 de dominio y los 12 del contrato citan su caso de la suite, su `e2e-<regla>` o su `regla_<slug>.test.mjs`.
## Otros comandos
