...
> evidencia (símbolo y línea, o regla del vault) y la lleva a la lista de decisiones. Todo monto es un
...
El catálogo real tiene tres capas. El **modelo** declara siete etapas en `STAGES` (l.912) con su
orden `STAGE_ORDER` (l.921). El **tenant** puede renombrarlas y colorearlas (`ETAPAS_TENANT_DEFECTO`
l.934, regla 28), y `ETAPA_PUBLICADA = "oferta_publicada"` (l.931) es un pseudo-estado que no es un
`stage`: es `oferta` con la oferta cerrada y publicada (`etapaVisualId` l.994). Después de la firma
manda un tercer traductor, `estadoOperacion` (l.1200), que no es configurable y gana sobre el rótulo
del tenant (`etapaDeDeal` l.1004 = `estadoOperacion(d) || stageName(etapaVisualId(d))`).

| Estado (id del modelo · rótulo del tenant) | Qué lo abre | Qué lo cierra | Quién actúa |
|---|---|---|---|
| `prospeccion` · **Sin gestión** / Prospección | La corrida horaria del inbound crea la oportunidad con la oferta vacía (`correrProceso` l.48622); o el rollover de fin de día la reabre (`rolloverDia` l.49407) | La **simulación** de la oferta (`simularOferta` l.50148, regla 12-bis); o la pérdida | Inbound (sistema); el ejecutivo arma la oferta |
| `oferta` · **Negociación** / Oferta y Negociación | `simularOferta` promueve `prospeccion → oferta` | Cerrar y publicar (`cerrarOferta` l.47286); la pérdida por oferta no aceptada o cesión a otro factor (`evaluarPerdidas` l.49293) | Ejecutivo; Agente IA (opcional, regla 8); apoderados si hay pre-evaluación |
| `oferta_publicada` · **Oferta publicada** (pseudo-estado) | `patchCierre` asienta `ofertaCerrada` + `ofertaComunicada` (`ofertaPublicada` l.23521, regla 58). El predicado tiene dos lecturas más: «cerrada» = `ofertaCerrada \|\| negocioNum` (l.23523) y «comunicada» = `ofertaComunicada` **o** algún mensaje de `waSesion` que calce `/Oferta de factoring/i` (l.23524), porque el Agente IA publica por WhatsApp sin la bandera (comentario l.23519–23520 y l.47320–47322; regla 8, caso 158): también entra al pseudo-estado una oportunidad con `negocioNum` cuya sesión de WhatsApp contenga la oferta del agente, sin pasar por `cerrarOferta` | La firma del cliente en el portal (`confirmarCierre` l.47637); reabrir (`reabrirOperacion` l.48405) vuelve a `oferta` | El cliente en el portal; comité (solicitud inyectada); apoderados y verificación (pendientes ya exigidos, regla 54) |
| `aceptadas` · **Aceptada** | Id del catálogo; `estadoOperacion` lo rotula igual que `cesion`. En el fuente no se encontró escritor de `stage: "aceptadas"` con esa cadena: la firma escribe `cesion` u `otorgamiento` según `etapaTrasFirma` (el `patch` de `confirmarCierre`, l.47771–47798) | — | — |
| `cesion` · **Aceptada** (rótulo) / **Pendiente Integración** (con `integracion: "pendiente"`) | `etapaTrasFirma` (l.23618) cuando la firma no deja nada pendiente: `{stage: "cesion", integracion: "pendiente"}`; también `moverEtapa` (l.48330, botón «Avanzar» del detalle) tras la compuerta OTG-02, pero **sin** `integracion` —`estadoOperacion` la rotula «Aceptada», no «Pendiente Integración»— (§13) | `aprobarIntegracion` (l.49692) tras OTG-01 y `controlesIntegracion` | Operaciones N3 |
| `otorgamiento` · **Otorgamiento / Verificación** | `etapaTrasFirma` cuando falta algo: motivo `linea_o_deudor` · `excepciones` · `verificacion` · `evidencia`; también `moverEtapa` y el arrastre del Kanban (`moveTo` l.48439), sin guarda (§13) | `otorgamientoCompleto` (l.23645) → el `useEffect` de avance (l.50629); la pérdida por bloqueo firme | Apoderados por (área, nivel); Ejecutivo de verificación; el ejecutivo (respaldos); Operaciones (O05 física) |
| `giro` con `giroPendiente: true` · **Pendiente de Giro** | `aprobarIntegracion` escribe `{stage: "giro", integracion: "aprobada", giroPendiente: true}` y congela el giro en `repoGiro` | El callback de Tesorería (`recibirGiroTesoreria` l.42253) | Tesorería (fuera de NEX, regla 43) |
| `giro` sin `giroPendiente` · **Girada** | El aviso de Tesorería devuelve el patch `{giroPendiente: false, giroRef, giroTs}` | Terminal | — |
| `perdida` · **Perdida** | Ocho escritores (§13): manual (`reject` l.46861), cron (`evaluarPerdidas`), bloqueo firme (`useEffect` l.50592) y dos manuales sin causa (`moverEtapa` l.48330, `moveTo` l.48439) | Terminal (regla 5): reabrir es una operación nueva con referencia | Ejecutivo; sistema |

Dos colapsos de pantalla que no son estados: `displayStageId` (l.1173) muestra `giro` como
`aceptadas` en el tubo, y `fueraDelTubo` (l.1210) saca de la lista lo que ya es de Operaciones.
...
|---|---|---|
| **Inbound (sistema)** | Clasifica el stream, agrupa por cedente, abre o actualiza la oportunidad, cierra el día y reabre lo no gestionado | `tickCron` (l.49359), `correrProceso`, `rolloverDia`; `spec-inbound-facturas.md` §6 |
| **Ejecutivo comercial** | Arma la oferta, pide la evaluación, justifica cada excepción, cierra y publica, puede reabrir, puede perder | `incorporarFacturasOferta` / `retirarFacturaOferta` (regla 33), `reevaluarLinea` (l.12456), `solicitarAprobacionExc` (l.25360), `cerrarOferta`, `reabrirOperacion`, `reject` |
| **Agente IA** | Canal opcional del primer contacto y de la publicación por WhatsApp; ofrece dentro del piso del deudor | `canalDeRegla` (`spec-inbound-facturas.md` §5.2), `SPREAD_MIN_DEUDOR` (l.2078), regla 8 |
| **Apoderados por (área, nivel)** | Aprueban o rechazan excepciones en la mesa Otorgamientos o en el tab del detalle; nivel igual o superior de la misma área | `aprobarExc` (l.10068) con OTG-01 antes de escribir; `puedeAprobarExc` (regla 18); `spec-gestion-excepciones.md` §2.2 |
| **Ejecutivo de verificación** | Contacta al deudor, firma verificada / no verificada por factura, retira lo no confirmado | `verificarDeudor` (l.50239), `DrawerVerificacion` (regla 53), `puedeVerificarFacturas` (regla 18) |
| **Comité de líneas** (sistema externo) | Resuelve la solicitud de líneas que el cierre inyecta; NEX sólo inyecta y consulta | `solicitudComiteDeOferta` (l.41387) → `api1Inyeccion`; `api3EstadoProceso` (l.42277); regla 15 |
| **Operaciones N3** | Visa O05 en la vía física; aprueba la integración al core tras los controles | `aprobarExc` (O05), `aprobarIntegracion` con `controlesIntegracion` (l.23685); regla 41 |
| **Tesorería** (fuera de NEX) | Gira y avisa | `recibirGiroTesoreria` (pura); regla 43 |
| **El cliente en el portal** | Acepta formalmente firmando con código de un solo uso; es la única aceptación válida | `enviarCierre` (l.47471), `confirmarCierre`; regla 1 |
...
- *Llegada.* `facturaDeDTE` (l.3949) lee del A1 las banderas `reclamada` (`EstadoDTE.Reclamado`),
...
- *Corrida horaria.* Un `setInterval` cada `CRON_MS` (l.49375–49377; `cronMs: 3500`, «1 «hora» = N ms»,
  l.4640; `frecuenciaMin: 60`, l.4639) llama a `tickCron` → `correrProceso` + `evaluarPerdidas`; sigue tras
  drenarse el stream (`spec-inbound-facturas.md` §8). Esa corrida es una **simulación en el navegador**:
  el `setInterval` sólo se instala si `CFG_ACTIVA.modoDemo !== false` (l.49374; comentario l.49370–49372:
  «El cron es parte del MOTOR DE SIMULACIÓN: sin modo demo no corre») y su período es `CRON_MS =
  cfgT.cronMs` (l.48518), 3,5 s. `frecuenciaMin: 60` es un **parámetro huérfano**, igual que `horaInicio`
  / `horaFin`: se edita en Configuración › Operación (`<input {...num("frecuenciaMin", …)}>`, l.34110) y
  ningún cron lo lee (`grep frecuenciaMin`: l.1054 comentario, l.4639 declaración, l.34110 pantalla). El
  job real del backend está pendiente (§10.4).
- *Candidatura.* `clasificarFactura` (l.4395) toma la **primera** regla activa de `INBOUND_RULES`
  (l.1929) cuyos criterios califican; un criterio desconocido califica todo (`facturaCalifica`). El
  filtro de calidad «Buena factura» (cuatro condiciones, `CRITERIO_PRED` l.4368) y el catálogo de
  ...
  ni «con NC», sólo excluirlas vía «Buena factura»—; los demás criterios de `CRITERIO_PRED`
  (l.4361–4392) —incluido `Crédito` (l.4370), que sí es seleccionable por sí solo, y los alias
  persistidos `Clientes` / `No clientes` (l.4383, l.4385), que existen por compatibilidad con las reglas
  guardadas en localStorage (comentario l.4374–4377: un criterio desconocido califica TODO)— son los de
...
- *Apertura y actualización.* `correrProceso` agrupa `acumulado` por cedente y crea la oportunidad
  ...
  tope `MAX_NUEVOS = 40` por corrida (l.48630), el resto se re-encola. Si el cliente ya tiene una
  ...
  (`aplicar(warn)`, l.48886); la oferta elegida no se toca. Si la existente está aceptada, cursada o
  ...
  en unos segundos» (l.13260) es **inalcanzable**: está condicionado a `deal.actualizando && !fullPage`
  (l.13252; lo mismo «Actualizando deudores de la oferta…», l.14252–14256) y el único sitio de render de
  `DealDrawer` pasa `fullPage` (l.50869–50872; el detalle es pestaña propia desde el 02-09-2026), así
  que es código muerto y la pantalla del detalle no anuncia nada. Lo que sí promete una re-simulación
  es el comentario de `correrProceso` («el resultado re-simulado llega tras una latencia», l.48862–48865),
  el estado `actualizando: true` (l.48874, que sólo dura la latencia) y los dos mensajes de bitácora
  `logSys` «Recalculando N oportunidad(es) por M documento(s) nuevo(s) · latencia estimada …» (l.48876)
  y «Recálculo aplicado» (l.48903); pero `aplicar` (l.48886–48899) sólo anexa a `facturasDisponibles` y
...
- *Segmentación Prime / Otros y join con líneas.* `capacidadDeudores` (l.41700) devuelve tres
  tramos con `n` y `monto`: `primeConLinea`, `otrosConLinea`, `sinLinea`. Prime = `tipoDeudor` Lista
  Blanca o Deudor Autorizado (`CRITERIO_PRED` l.4376, `esPrime` l.31339). «Con línea» se decide en el
  ...
  dibuja con `chipTramo` (declarado l.20940, primera llamada l.20981) y el tooltip del chip `primeConLinea` (l.20986) lo declara
  **cota superior**, «no lo que se va a girar». El nivel cliente viene de `lineaCreditoDe` (l.3777).
  Esa cuantificación **no es un dato de la oportunidad**: `correrProceso` no la escribe (l.48622–48720
  no calcula prime/otros ni línea) y `capacidadDeudores` tiene un solo llamador, la tarjeta del tubo
  al dibujarse (l.20933), donde es un lookup contra el listado A23 («no una corrida del motor»,
  l.20929–20932). En producción no habría dónde consultarla fuera de la pantalla.
- *Cierre del día.* No hay hora de reloj en la lógica, aunque sí como **parámetro huérfano**:
  ...
  de operación (inbound + actualizaciones)» (l.4635–4636), Configuración › Operación las edita con
  `<input type="time">` (l.34098, l.34102) y ningún otro sitio del fuente las lee (`grep horaInicio|horaFin`:
  cuatro resultados, todos de configuración). El cierre se dispara por conteo: `corridas % HORAS_DIA
  === 0` (l.49527) → `cerrarDiaRef.current` (l.49504) → `rolloverDia(nDia)`. Con
  `reaperturaDiaria: true` (l.4650) y `etapaNoGestionada: "prospeccion"` (l.4651), toda oportunidad
  `_inbound` que quedó en esa etapa se **reabre con el mismo id** (comentario del fuente «EL ID NO
  CAMBIA», l.49426), con el paquete re-escalado, `facturasOp: []`, `simulado: false` y
...
|---|---|---|
| M-01 | implementado distinto | Las cesiones llegan por A2, no por DTESync; NC sin monto en el A1; **«Aceptaciones» no existe** como campo ni regla en ningún activo ni spec (el `EstadoDTE` del A1 trae sólo `Reclamado`, `NotaCredito`, `FolioNotaCredito`, `facturaDeDTE` l.3949–3971); en el fuente sólo como dato **sintético sin consumidor**: `facturasDeCandidata` (l.30290) sortea `estado: "Aceptada" \| "Reclamada" \| "Sin acuse"` por RNG (l.30312, única aparición de «Sin acuse»), que habría que retirar o conectar al A1 si el modelo confirma que la aceptación (acuse de recibo, Ley 19.983) es criterio de candidatura |
| M-05 | implementado distinto | La cuantificación se calcula en el render de la tarjeta (`capacidadDeudores`, único llamador l.20933), no se persiste en la oportunidad ni la produce el inbound. «Prime» acá es sólo listas; en verificación el protocolo recortado es prime **o** nota > 4,2 (regla 6). El modelo debe fijar cuál «Prime» segmenta la oportunidad |
...
| M-07 | implementado distinto | Corte por N corridas, no por hora (la ventana `horaInicio` / `horaFin` es un parámetro declarado sin efecto, l.4635–4636); y `rolloverDia` **reabre** con el mismo id, no elimina. `spec-ciclo-factura.md` §17 (l.1063–1066) dice que el rollover «cierra y **re-origina** las oportunidades del inbound que nadie gestionó» y que «Es una oportunidad **nueva**, con su propio identificador»: contradice al fuente («EL ID NO CAMBIA», l.49426) y a la regla 22 (§15) |
| M-08 | **pendiente** (con parámetro huérfano) | La ventana `horaInicio` / `horaFin` (l.4635–4636) se edita en pantalla (l.34098, l.34102) y nadie la consume: `rolloverDia` reabre en el mismo tick del cierre. Falta que el corte 23:00 y el reinicio 06:00 sean parámetros y que el job los use; hoy la ventana configurada es decorativa |
| M-09 | implementado distinto | «Buena factura» **no** consulta `cedida`; la cesión bloquea sólo al incorporar (`estadoCandidata` l.6120) y como pérdida por AECSync. Y la candidata exige «a crédito», que el modelo no menciona |
| M-10 | implementado distinto | No existe criterio por **fecha de emisión** (el descarte < 8 días ocurre al **cerrar**, `intentarCerrar` l.12483; al publicar «ya se resolvió al cerrar», l.12472–12473), ni por **cesión previa**; reclamo y NC sólo como parte de «Buena factura», no seleccionables; ni una **lista de emisores con tags**: los tags son del deudor (A3/A4) y del cedente sólo `esCliente` / SOW |
...
`seleccionConLinea` (l.41745) / `facturasConLinea` (l.41758) devuelven `CON_LINEA`. `estadoCandidata` (l.6120)
...
saca la oportunidad de Prospección es la **simulación** (`simularOferta` l.50148, regla 12-bis), no
editar el paquete.
...
verificación hasta apretar Re-evaluar (`reevaluarLinea` l.12456; regla 14, gate 124, e2e-14).
...
hay una sola pasada**, y **no hay un evento de evaluación que invoque los motores**: `simularOferta`
(l.50148–50186) sólo escribe `simulado`, `stage` y `finanzasDe` sin llamar a `asignarLineas` ni a
`verifDecision`; «Re-evaluar operación» (cabecera, `reevaluarLinea` l.12456–12462) es un spinner de
700 ms que apaga `reevalPend` y con eso el render del detalle vuelve a correr líneas (`evalLin`, l.14349),
verificación (`verifFactura` l.3354) e ítems, sin emitir versión; «Re-evaluación de la simulación» (tab Otorgamiento, `reevaluarCliente` l.23369) pide
...
(`tasaPond`, l.13543). Las fichas dicen sólo lo que este documento necesita; los catálogos y las
...
| | |
|---|---|
| Cuándo corre | Simulación (evalúa pero **no emite versión**: `spec-ciclo-factura.md` §23c fila 5; la v1 se emite recién con la primera «Re-evaluación de la simulación», `reevaluarCliente` l.23373; `spec-gestion-excepciones.md` §4.1 la llama «versión v1» y está desactualizado), pre-evaluación, «Re-evaluar operación», «Re-evaluación de la simulación», cierre de la oferta (compuerta) y firma del cliente (`spec-otorgamiento.md` §2, l.39–46, lista cuatro momentos —pre-evaluación, una sola «Re-evaluación» disparada por «Re-evaluar operación», cierre y firma—: omite la simulación, no distingue los dos gestos que este documento separa y está desactualizado en un segundo punto: atribuye a «Re-evaluar operación» «veredicto vigente + versión nueva (evidencia)», cuando en el fuente ese botón (`lanzarReeval` l.9176–9181 → `onReevaluar` l.15619 → `reevaluarLinea` l.12456–12462; cabecera l.15741) no toca `repoSimVersions` y la versión la emite sólo `reevaluarCliente` (l.23369, «Re-evaluación de la simulación» l.10680; los únicos `repoSimVersions.push` son l.23373, l.23375 y l.50423); lo correcto es `spec-gestion-excepciones.md` §5.5 («No crea una versión», l.526) y §4.1) |
| Unidad de decisión | La regla × el sujeto: reglas C/O una vez por operación, reglas D una vez por deudor; `stKey` = `n` o `n@RUT` (`evaluarOtorgItems` l.22328, clave en l.22345) |
...
| Qué devuelve | `items[]` con disposición `aprobado · excepcion · rechazado · clasificacion · no_ejecutada`, área, nivel exigido tras el piso por monto, aprobadores; agregado `aprobada · sujeta · rechazada` (`visadoDealCalc` l.23431) |
| Qué se versiona | `vars` y `res` (disposición por regla) en `snapVersionCli` (l.23157); el visado **no** va en la versión, vive en `repoVisado` por `stKey` (§8) |
...
`NO_REEV_CLIENTE` es la lista de las **no** re-evaluables (`reglaReev = (n) => !NO_REEV_CLIENTE.has(n)`,
l.21623) y `rechReev` es su complemento; el literal de l.21622 (n 12–41 del catálogo viejo) está muerto,
porque la IIFE que inyecta el Modelo de Riesgo v1.0 hace `NO_REEV_CLIENTE.clear()` y la rellena con los
28 ids del catálogo vivo (l.22954–22957): cuáles son lo dice `spec-gestion-excepciones.md` §2.4, no este
documento. Los únicos tramos «rechazado»
del catálogo vivo son `tHard` en C30/C31/C32 (l.22646–22655): eso es lo que sostiene «rechazo firme sólo
C30–C32».
...
| | |
|---|---|
| Cuándo corre | En el **render del detalle** (`verifFactura` l.3354), en la misma pasada que líneas (regla 14, gate 124), y con evidencia sólo en `snapVersionCli.verificacion` al hacer «Re-evaluación de la simulación»; ningún evento de evaluación la invoca: `simularOferta` (l.50148–50186) no llama a `verifDecision` y «Re-evaluar operación» sólo apaga `reevalPend` (§7); al cerrar y publicar aparece el tab (regla 58) |
| Unidad de decisión | El **deudor** dentro de la operación; la verificación de una factura es la de su deudor (`verifDecision` l.3212, regla 6) |
| Qué recibe | El par cliente-deudor y **todas** las facturas de ese deudor en la operación (`verifFactura` l.3354); el estado del cliente por parámetro (regla 6, «Regla 0» de verificación; `spec-verificacion-facturas.md` §4.0) |
| Qué devuelve | `requiere_verificacion` con motivo y la lista completa de criterios evaluados (`spec-verificacion-facturas.md` §2.3); por operación, `verifResumenDeal` (l.3430) cuyo `.pend` es VER-01 |
| Qué se versiona | El veredicto por deudor en `snapVersionCli.verificacion`; el contacto vive aparte en `repoVerifTel` (por factura: `{por, fecha, checklist{existencia, recepcion, fechaPago}, contacto, compromiso, respaldo, sinRespaldo, notas}`, `verificarDeudor` l.50248–50258) y el veredicto congelado en `repoVerifVeredicto` (por deudor, l.24174 y `congelarVeredicto` l.50387) |
...
| | |
|---|---|
| Cuándo corre | En el **render del detalle** (`evalLin`, l.14349) y en cada versión (`snapVersionCli` l.23244, sólo desde `reevaluarCliente` l.23373–23375 y `retirarFacturaOferta` l.50423), recálculo siempre completo (regla 7); no la invoca `simularOferta` (l.50148–50186, que sólo escribe `simulado`, `stage` y `finanzasDe`) ni «Re-evaluar operación» (`reevaluarLinea` l.12456–12462, un spinner de 700 ms que apaga `reevalPend`); al cerrar la oferta para armar la solicitud (`cerrarOferta` l.47286, llamada a `asignarLineas` l.47421); LIN-01 factura por factura al integrar (regla 41) |
| Unidad de decisión | La **factura completa**: `CON_LINEA` o `REQUIERE_COMITE`, nunca parcial (`asignarLineas` l.41762; l.41888 y l.41932) |
| Qué recibe | Facturas itemizadas, RUT cliente, estado de líneas inyectado (`inyecta`) |
| Qué devuelve | `cursable`, `requiereComite`, `facturas` con `origen[{lineaId, tipo, monto}]` (una factura sí puede repartirse LF3 → LF2, l.41898), `deudores`, `lineasUsadas`, `solicitudes` con motivo `par · lf4 · lf1 · cliente · deudor · suspendida` |
| Qué se versiona | `snapVersionCli.linea` completa; `lineaDeVersion` (l.41667) pinta desde la última versión sin volver a correr el motor (regla 13) |
...
| Unidad de decisión | El **deudor**; las facturas heredan (`asignarGiros` l.5471, regla 22) |
...
| Qué se versiona | No va en `snapVersionCli`; se congela aparte al inyectar (`giroCongelado` l.24447 gana sobre el cálculo del día) |
...
| | |
|---|---|
| Cuándo corre | Al simular (`simularOferta` → `finanzasDe`) y en el render del detalle (`proRiesgo` l.13531 / `tasaPond` l.13543); el motor de conceptos `simularOperacion` (l.5152) es puro |
| Unidad de decisión | La **operación** para el desglose; el **deudor** para la tasa (`spreadSugerido` l.2145 con piso `SPREAD_MIN_DEUDOR` l.2078); la **factura** para el prorrateo (`prorratearOperacion` l.5322, regla 21) |
| Qué recibe | Catálogo de conceptos y constantes del tenant por parámetro; política comercial (spread de lista, tabla SOW); costo de fondo; `tasaModo` (l.4653) |
| Qué devuelve | Filas del desglose y el cierre fijo del catálogo (`spec-pricing-simulacion.md` §3.1 «El cierre es fijo»; regla 20); tasa y plazo equivalentes (`tasaEquivalente` l.5276); giro por factura |
| Qué se versiona | **Nada de las condiciones comerciales**: ni `snapVersionCli` ni `huellaOperacion` (l.24201) contienen tasa, comisión ni anticipo (regla 23) |
| Spec propio | `Specs_Procesos/Evaluacion_Factura/spec-pricing-simulacion.md` |

La comparación contra el último negocio existe y es del **cliente, no del par**: `historialComercial`
(l.6736) siembra sólo por cliente (`hashStr(cliente + "hc")` l.6737 y `hashStr(cliente + "_op_" + i)`
l.6741) y el parámetro `deudor` sólo rotula cada fila (l.6750); `tasaUltimoNegocio` (l.6762) llama
...
la operación») y el sistema decide así (`verifDecision` l.3212; `verifFactura` l.3354 evalúa todas las
facturas del deudor): decisión abierta (§15); (e) el contacto de verificación **sí registra** los tres
hechos que el modelo pide obtener: `DrawerVerificacion` (l.7215) lleva el checklist `{existencia,
recepcion, fechaPago}` (l.7218; rótulos «Existencia de la factura», «Recepción conforme», «Fecha de
pago», l.7312–7316; en la mesa `CHECKS`, l.11555) y exige los tres más la fecha de pago comprometida
antes de confirmar (`completo = chk.existencia && chk.recepcion && chk.fechaPago && !!compromiso`,
l.7229), y `verificarDeudor` (l.50239) lo persiste por factura en `repoVerifTel` (l.50248–50258), de
donde `verifFactura` lo lee (`checks`, l.3377–3383). Lo pendiente es sólo documental: `spec-verificacion-facturas.md`
no describe el checklist y el comentario «guarda `{por, fecha}` y nada más» de l.3367 quedó obsoleto
(M-22-bis, §14 y §16).
...
`repoSimVersions` (l.24287) con `snapVersionCli` (l.23157); la simulación inicial **no** emite versión
(`spec-ciclo-factura.md` §23c fila 5; `simularOferta` l.50148 no toca `repoSimVersions`). La
**primera** re-evaluación emite **dos**: `reevaluarCliente` hace `if (!vs.length)
repoSimVersions.push(deal.id, snapVersionCli(deal, 0))` —«persiste la evaluación inicial (v1)»,
l.23373— y luego la nueva (l.23375). Esa v1 es **retroactiva**: se calcula con los datos del momento de
la re-evaluación, no de la simulación, así que la evidencia de la evaluación inicial no es
contemporánea (los únicos `push` son l.23373, l.23375 y l.50423). El retiro por verificación emite una
versión con la línea recortada (`retirarFacturaOferta` l.50423). Desde Aceptada en adelante una versión
nueva no re-asigna: recorta la anterior (`recortarAsignacion` l.41572, llamada en `snapVersionCli`
l.23244).
...
| **Requisitos de excepción** (ítems con disposición, área, nivel, aprobadores) | `snapVersionCli.res` (disposición por regla) + `repoVisado` (l.24142) / `VISADO_STATE` (l.24462) por `stKey` + `SOLICITUD_EXC` (l.24466) | La versión es foto; el **visado sobrevive por clave estable** (`reevaluarCliente` l.23376: «NO se tocan las excepciones ya resueltas»). Si la regla deja de gatillar, el ítem sale aprobado y el visado, la solicitud, el hilo y la tarea quedan **huérfanos** en el repositorio: no se eliminan ni se marcan (M-21) |
| **Requisitos de verificación** (por deudor, con causas) | `snapVersionCli.verificacion` + `repoVerifTel` por (operación, factura) + `repoVerifVeredicto` por (operación, deudor) | El contacto y el veredicto congelado sobreviven por clave estable; `limpiarSimulacion` (l.50187) y reabrir no los tocan (regla 1). No es versionado: es clave estable (M-25) |
...
| **Huella del paquete** | `huellaOperacion` (l.24201): `op · rut · nd · nf · monto · deudores[rut:monto]` + SHA-256, en `repoContratoEvidencia` | Si el paquete cambia, la huella no calza y O05 vuelve a ser excepción por sí solo (regla 23) |
...
contemporánea de la simulación (l.23373). M-20 y M-25
...
«huérfan» —sólo en Configuración › Áreas, l.35724 y l.35862, sobre criterios que rutean a un área
...
`ev.requiereComite > 0` (l.41389).

**Qué hace el sistema hoy.**

- *Justificar.* `solicitarAprobacionExc` (l.25360) **no exige nada**: recibe `(deal, x, execCode,
  comentario, archivos, sinComentarios)` y guarda `comentario || ""`, `(archivos || []).slice()` y
  `!!sinComentarios` sin validar (l.25368–25370). La exigencia —comentario, respaldo adjunto o la
  declaración explícita «sin comentarios»— vive en el formulario del tab (llamadas l.10653 con
  `sf.msg, sf.archs, sf.sinCom` y l.11018 con `"", [], true`); el botón de Pre-evaluación (l.13193) la
...
- *Cerrar y publicar.* Es una sola decisión en `ModalCurse` (regla 23) con dos compuertas: sin
  excepciones sin justificar y Monto a Girar positivo, corte en $1 (`giroCursable` l.11952, regla
  13-septdecies). **Las dos compuertas no están al mismo nivel**: la del monto se re-comprueba en la
  mutación (`cerrarOferta` l.47293–47296, con el comentario «la pantalla que apaga el botón no es el
  control (regla 24)»); la de las excepciones sin justificar vive **sólo en la pantalla** —`disabled=
  {sinComentario > 0 || !gOk}` del botón de `ModalCurse` (l.12293) y `if (!datosCurse) return;` en
  `ejecutarAccion` (l.12745)—, y entre l.47300 y l.47400 `cerrarOferta` no consulta `excPend`,
  `sinComentario` ni el visado (§16). El CTA nombra lo que va a pasar: «Enviar a Comité y Publicar» cuando parte de la
  oferta no tiene cupo (`spec-ciclo-factura.md` §6). `cerrarOferta` (l.47286) escribe `patchCierre`
...
- *Enviar a comité.* Lo genera el cierre: `asignarLineas` → `solicitudComiteDeOferta` (l.41387: una
  solicitud con N líneas de detalle `tipoLinea: "puntual"`, una por deudor sin cupo, `propFactoring =
  aprobada + pedido`) → `api1Inyeccion` (l.42185) → `repoSolicitudComite` y `deal.solicitudComite`;
  ...
  "agregar_deudores"`, `propGlobal: 0` y cada línea de detalle con `tipoLinea: "puntual"`
  (l.41394–41411); `RESOLUCION_COMITE` (l.41366) sólo aporta el rótulo `pide` / `alcance` que viaja en
  `detalle[]` (`par` / `lf4` → puntual; `lf1` → asignación por comité; `cliente` / `deudor` → global;
  `spec-ciclo-factura.md` §11), y `api1Inyeccion` (l.42185) inyecta ese objeto sin transformarlo: una
  factura rechazada por `cliente` o `deudor` produce igual una puntual cliente-deudor.
- *Bloqueo firme.* `visadoDealCalc` (l.23431) marca `rechFirme` sólo con rechazos no re-evaluables
  (C30–C32 del cliente); `bloqueoFirmeInfo` (l.23493) arma la causa y el `useEffect` de l.50592 pasa
  la operación **entera** a `perdida` con `perdidaPor: "sistema"`. `retirarFacturaOferta` (l.50388)
  tiene tres llamadores y los tres son de verificación con motivo `"noConfirmada"` (l.50263, l.50318,
  l.50363): ningún camino retira facturas por otorgamiento.
...
devuelve `null` salvo `ev.requiereComite > 0` (l.41387–41389) y `cerrarOferta` sólo inyecta si hay
solicitud (l.47421–47441), y la cláusula pasa a «implementado distinto». **M-19 implementado
...
- *La firma es del cliente y sólo en el portal.* `enviarCierre` (l.47471) abre la pestaña del portal
  en el gesto del clic, emite el OTP una vez (sólo el hash queda en el store) y envía el enlace; el
  correo es aviso sin enlaces. `confirmarCierre` (l.47637) recibe el `postMessage` de la firma, registra
  la evidencia del contrato por la vía electrónica (`registrarEvidenciaContrato`), calcula la oferta,
  recorta a `montoFirmado`, arma `dealFirmado` (l.23607: `stage: "cesion"`, `clienteAcepto: true`,
  `reabierta: undefined`) **sólo como base en memoria** sobre la que se evalúan las compuertas
  (`visadoDeal`, `verifResumenDeal`, `evidenciaContratoOk`, l.47708–47712) y llama a `etapaTrasFirma`
  (§11). Lo que se **persiste** es el `patch` de l.47771–47798: `stage: stageFinal` —`"cesion"` sólo
  cuando `destino.integracion` (l.47734), con `otorgada: true, integracion: "pendiente", giroPendiente:
  true` (l.47736), y `"otorgamiento"` con `giroPendiente: true` en cualquier otro caso (l.47759–47761)— más
...
- *Física vs electrónica.* O05 «Contrato firmado por cliente de la operación» (#305, Operaciones N3,
  l.22923) existe siempre. Electrónica: la firma en el portal es la evidencia y O05 queda aprobado
  sin visado. Física: O05 queda como excepción de Operaciones N3, el ejecutivo adjunta el comprobante
  y `aprobarExc` (l.10068) registra la evidencia al aprobarlo; revertir el visado revoca la evidencia
  (l.10158). Entre las dos vías cambian 0 de 77 criterios (`spec-ciclo-factura.md` §16).
- *La evidencia es una huella.* `huellaOperacion` (l.24201), cadena canónica + SHA-256; las
  condiciones comerciales no entran (regla 23). `aprobacionFormalCliente` (l.23581) es la lectura:
  ...
  `clienteAcepto: true` es el manejador del intent `cursar` del WhatsApp del cliente (l.48046–48083):
  ...
  enlace de cierre enviado» (l.48080), y desde ahí `aprobacionFormalCliente` devuelve `true` sin firma
...
- *Reapertura.* `reabrirOperacion` (l.48405) comprueba `edicionOperacion` (regla 33), guarda
...
- *La bifurcación.* `etapaTrasFirma` (l.23618) recibe `requiereOtorg` (línea o deudores «Otro»),
...
- *Otorgamiento post-firma.* Los apoderados deciden en la mesa o en el tab (`aprobarExc`, OTG-01
  antes de escribir); el tab sobrevive a la firma. `otorgamientoCompleto` (l.23645) = en
  ...
  por sí misma.
- *Verificación post-firma.* `verificarDeudor` (l.50239) registra las confirmadas en `repoVerifTel`
  ...
  congela con el contacto (`congelarVeredicto` l.50387) y el predictor no vuelve a opinar.
- *Líneas del comité.* NEX sólo consulta (`api3EstadoProceso` l.42277, pull con «Consultar
  ...
  integración (`controlesIntegracion` l.23685). La única acción es manual, «Sacar facturas sin línea ·
  N fact.» (l.16088, en la pestaña de deudores del detalle, no en el menú Acciones; el estado vacío dice
  «No hay facturas por agregar ni facturas sin línea por sacar.», l.16067; comentario l.20541), con la
  oferta abierta. `spec-asignacion-lineas.md` §8.7 (l.636) la llama «Quitar lo que no tiene línea»:
  rótulo desfasado del fuente.
- *Salida de la etapa (desfase medido).* Dos escritores: `avanzarPipeline` (l.48913) movería
  `otorgamiento → cesion + integracion pendiente` exigiendo `verifResumenDeal.pend === 0`, pero su
  `avanzarRef.current` **nunca se invoca**; y el `useEffect [deals, cfgVer]` (l.50629) con
...
- *GE / GN.* `asignarGiros` (l.5471): GE / GN según el catálogo declarativo de tipos y sus cuatro
...
- *Inyección al core.* `aprobarIntegracion` (l.49692): OTG-01 → `controlesIntegracion` (l.23685)
...
- *La noticia de Tesorería.* Girar no es una acción de NEX (regla 43): `moverEtapa` no ofrece `giro`
  como transición manual, GIR-01 queda `aplicado: "externo"`. `recibirGiroTesoreria(ev, deal)`
  (l.42253) es pura: exige `operacionId`, `stage: "giro"` y `giroPendiente`; devuelve
...
(`causaPerdidaDeal` l.23505–23518: `causaPerdida` → `bloqueoFirmeInfo.causa` → «Cesión externa» → «El
cliente no aceptó la oferta» → «Sin contacto» → «Oportunidad perdida (causa no registrada)» **sólo si el
status es el genérico** «no superó reglas de otorgamiento» (l.23516) → `deal.status || "Oportunidad
perdida"` (l.23517). Y como `STATUS_ETAPA` (l.5834–5841) no tiene clave `perdida`, los dos escritores
manuales escriben `status: STATUS_ETAPA["perdida"] || d.status` = el status de la etapa anterior
(`moverEtapa` l.48394, `moveTo` l.48475): para una pérdida por arrastre o por «Avanzar»,
...
|---|---|---|---|
| `reject(id, closeReason, extra)` l.46861 | Manual desde el detalle; deriva `bloqueo_firme` si hay `bloqueoFirmeInfo`, si no `cliente_declino`; guarda `cedidaCompetidor` / `tasaCierreCompetidor` | Ejecutivo | vivo |
| `evaluarPerdidas` l.49293 (cron) | Cesión al competidor por AEC, `rndDetBool` 12% una vez por oportunidad | Sistema | vivo |
...
| `useEffect` l.50592 | Bloqueo firme de otorgamiento (`rechFirme` o excepción rechazada por un apoderado): `perdidaOtorg`, `motivoPerdida: "bloqueo_firme"`, `perdidaPor: "sistema"` | Sistema | vivo |
| `avanzarPipeline` l.48973 y l.49168 | Cesión a la competencia documento a documento (sólo si todas las facturas están cedidas); «el cliente no aceptó» por `d.perdedor` | Sistema | **sin llamador** |
| `moverEtapa(id, stageId)` l.48330 | Botón «Avanzar» del detalle (`onMover(deal.id, avanzarA)` l.12693; `avanzarA` = siguiente id de `STAGE_ORDER` salvo `aceptadas`, l.12347–12350). Escribe `{...d, stage: stageId, status: STATUS_ETAPA[stageId]}` (l.48394) para cualquier etapa salvo `aceptadas` (l.48335) y `giro` (l.48358); `cesion` sólo pasa OTG-02 (l.48359–48373) y entra **sin** `integracion`; `otorgamiento`, `perdida` (**sin `causaPerdida` ni `etapaPerdida`**, contra la regla 5, y con `status` de la etapa de origen porque `STATUS_ETAPA` no tiene `perdida`) y volver a `oferta` / `prospeccion` no tienen guarda | Ejecutivo | vivo |
| `moveTo(stageId)` l.48439 | Arrastre del Kanban (`onDrop(stage.id)` l.8011). Escribe `stage: stageId` (l.48475) bloqueando sólo `aceptadas` (l.48442), la salida de `perdida` (l.48451) y el retroceso desde aceptadas / cesion / giro (l.48455); a `perdida` entra sin causa y con `status: STATUS_ETAPA["perdida"] \|\| d.status` = el de la etapa de origen | Ejecutivo | vivo |
...
reversible); el sistema tiene el rechazo firme sólo en tres reglas del **cliente** (C30–C32, `tHard`
l.22646–22655, rótulo «bloqueo firme») y su efecto no es «impedir avanzar» sino **pérdida automática y
terminal** de la operación entera (`useEffect` l.50592–50607 escribe `stage: "perdida"` con
`perdidaPor: "sistema"` para toda operación activa con `bloqueoFirmeInfo`; `snapVersionCli` l.23219
...
`vault/conocimiento/reglas/<tema>.md`; el motor que gobiernan las detalla en su spec. Donde el índice
está desfasado de la suite por la renumeración del merge de la sesión paralela —fila 8 («119, 147»),
15-bis-bis («126, 144») y 43 («145, 146»), y el texto de las reglas 43 («caso 145») y 15-bis-bis («Caso
144»)—, la columna Gate sigue la **medición** contra `tests_asignacion_lineas.js` (regla núcleo 2): el
144 es la regla 41, el 145 es ATR-01, el 146 es la colisión de `PRC-2601` entre pestañas, el 147 es el
callback de Tesorería, el 148 el congelado en la inyección y el 149 el mínimo del deudor; el índice y
esas dos reglas se corrigen en el mismo commit.
...
evidencia es símbolo y línea del fuente al 22-09-2026, o regla y spec.

| Id | Cláusula | Estado | Evidencia |
|---|---|---|---|
| M-01 | Las facturas llegan por DTESync con cesiones, NC, aceptaciones y reclamos | implementado distinto | `facturaDeDTE` l.3949 (reclamo y NC como banderas de `EstadoDTE`); cesiones por A2 (`spec-ciclo-factura.md` §1); «Aceptaciones» no existe en el A1 ni en ningún spec; en el fuente sólo como dato sintético sin consumidor (`facturasDeCandidata` l.30312: `"Aceptada" \| "Reclamada" \| "Sin acuse"`) |
| M-02 | Cada hora corren los motores de inbound | implementado distinto | La corrida horaria es una **simulación en el navegador**: `setInterval(…, CRON_MS)` l.49375–49377 sólo si `CFG_ACTIVA.modoDemo !== false` (l.49374; comentario l.49370–49372) → `tickCron` l.49359 → `correrProceso` l.48622; `CRON_MS = cfgT.cronMs` l.48518 (`cronMs: 3500` l.4640). `frecuenciaMin: 60` (l.4639) es un **parámetro huérfano**: se edita en Configuración › Operación (l.34110) y ningún cron lo lee, como `horaInicio` / `horaFin`. El job real del backend está pendiente (`spec-inbound-facturas.md` §10.4) |
| M-03 | Al detectar facturas se abre una oportunidad al cliente | implementado | `correrProceso` agrupa por cedente; tope `MAX_NUEVOS = 40` l.48630; `spec-inbound-facturas.md` §6 |
| M-04 | Si ya existe una oportunidad abierta se actualiza | implementado | `aplicar(warn)` l.48886: suma a `facturasDisponibles` deduplicando; la oferta no se toca; aceptada/cursada/perdida → otra |
| M-05 | Cuantifica facturas segmentadas en Prime y Otros | implementado distinto | `capacidadDeudores` l.41700 tiene un solo llamador, la tarjeta del tubo al dibujarse (l.20933, lookup sobre A23, «no una corrida del motor» l.20929–20932): se calcula en el render, no se persiste en la oportunidad ni la produce el inbound (`correrProceso` l.48622–48720 no la escribe); en producción no habría dónde consultarlo fuera de la pantalla. `chipTramo` l.20940; Prime = listas (`CRITERIO_PRED` l.4376) |
| M-06 | Join con líneas: con línea, Prime con línea, otros sin línea | implementado distinto | `capacidadDeudores`: `primeConLinea · otrosConLinea · sinLinea`, cota superior (tooltip l.20986); se calcula en el render de la tarjeta, no en la oportunidad (M-05); `lineaCreditoDe` l.3777 |
| M-07 | Hora de corte 23:00 elimina las no gestionadas | implementado distinto | Sin hora de reloj en la lógica (`horaInicio` / `horaFin` l.4635–4636 son parámetro declarado sin efecto); `corridas % HORAS_DIA === 0` l.49527 → `rolloverDia` l.49407 **reabre** con el mismo id; `reaperturaDiaria` / `etapaNoGestionada` l.4650–4651 |
| M-08 | A las 06:00 se reinicia el proceso | pendiente (parámetro huérfano) | Buscado `06:00`, hora de reinicio: nada. `CFG_OPER_BASE.horaInicio = "08:00"` / `horaFin = "18:00"` (l.4635–4636, «ventana horaria de operación (inbound + actualizaciones)») se editan en Configuración › Operación (l.34098, l.34102) y ningún job los lee: el cron es `setInterval(…, CRON_MS)` (l.49375–49377) y el cierre es `corridas % HORAS_DIA === 0` (l.49527); `rolloverDia` reabre en el mismo tick del cierre |
| M-09 | Candidata si no cedida, no reclamada, sin NC | implementado distinto | «Buena factura» l.4368 no consulta `cedida`; `estadoCandidata` l.6120 la bloquea al incorporar; exige además `credito` |
| M-10 | Reglas por RUT emisor, fecha de emisión, reclamo, NC, cesiones previas; lista de emisores con tags | implementado distinto | `INBOUND_RULES` l.1929 + `CRITERIO_PRED` l.4361; sin criterio por fecha ni cesión previa; reclamo y NC sólo dentro de «Buena factura» (l.4368), no seleccionables por sí solos; tags del deudor, no del emisor; criterio desconocido califica todo (l.4394) |
...
| M-12 | Cada cambio de selección re-evalúa | decisión abierta | Regla 14 (gate 124, e2e-14): la re-evaluación se pide; `reevaluarLinea` l.12456; `spec-otorgamiento.md` §2, `spec-asignacion-lineas.md` §8.4, `spec-ciclo-factura.md` §3 |
| M-13 | Cinco motores en paralelo, independientes | implementado distinto | Independientes (`spec-ciclo-factura.md` §0 punto 1) pero tres gestos de evaluación (`reevaluarCliente` l.23369; `spec-gestion-excepciones.md` §4.1, §5.5); pricing en el render (l.13531) |
| M-14 | Otorgamiento evalúa por empresa, no por factura | implementado | `evaluarOtorgItems` l.22328, `stKey` l.22345; `deudorBlock` l.22258; regla 4 |
| M-15 | Reglas bloqueantes impiden avanzar | implementado distinto | El modelo define «bloqueante» como lo que detiene la operación hasta retirar las facturas del deudor (M-18: por deudor y reversible); el sistema tiene rechazo firme sólo en C30–C32 del **cliente** (`tHard` l.22646–22655, «bloqueo firme»; `visadoDealCalc` l.23431 `rechFirme`) y su efecto es **pérdida automática y terminal** de la operación entera (`useEffect` l.50592–50607, `perdidaPor: "sistema"`; `snapVersionCli` l.23219 `estado = "rechazada"`; reglas 4 y 5), sin vía de destrabe |
| M-16 | Reglas re-evaluables | implementado | `NO_REEV_CLIENTE` es la lista de las **no** re-evaluables (`reglaReev` l.21623), vigente en l.22955 con los 28 de `spec-gestion-excepciones.md` §2.4 (el literal de l.21622 está muerto); `rechReev` es su complemento → sujeta; `reevaluarCliente` l.23369 emite versión |
| M-17 | Reglas excepcionables por apoderado según criticidad | implementado | Ruteo (área, nivel) + piso `PISO_ATRIB_MONTO` / `CFG_TRAMOS` (regla 4); `puedeAprobarExc` (regla 18); `aprobarExc` l.10068 con OTG-01 |
| M-18 | Deudor bloqueado → el sistema retira sus facturas automáticamente | pendiente | `retirarFacturaOferta` l.50388: tres llamadores, todos `"noConfirmada"`; el bloqueo firme pierde la operación entera (l.50592); no hay knockout de deudor (`spec-gestion-excepciones.md` §2.4) |
| M-19 | Justificar cada excepcionable antes de enviar a comité y publicar | implementado distinto | La justificación la exige la pantalla del tab (l.10653, l.11018), no `solicitarAprobacionExc` l.25360 (guarda sin validar); la Pre-evaluación solicita todas las pendientes sin justificar (l.13193); `cerrarOferta` l.47286 re-comprueba sólo el monto (`giroCursable`, l.47293–47296) y la compuerta de excepciones vive sólo en `ModalCurse` (l.12293; regla 30): falta la guarda en la mutación (regla 24); CTA «Enviar a Comité y Publicar» |
| M-20 | Excepción resuelta no se vuelve a pedir; versionadas | implementado distinto | `reevaluarCliente` l.23376–23378; `repoVisado` (l.24142) por `dealId + stKey`, `VISADO_STATE` l.24462; sobrevive por clave estable, no dentro de la versión |
| M-21 | Si ya no era necesaria, se marca cumple y se elimina | implementado distinto | `visadoDealCalc` l.23465 consulta sólo lo que gatilla hoy; la entrada en `repoVisado`, `SOLICITUD_EXC` l.24466, el hilo y la tarea quedan huérfanos |
| M-22 | Verificación por empresa para decidir el contacto | decisión abierta | Modelo: «empresa emisora» (= cedente); sistema: por deudor (`verifDecision` l.3212; `verifFactura` l.3354 evalúa todas las facturas del deudor); regla 6; `spec-verificacion-facturas.md` §2.1 («La decisión es por deudor dentro de la operación, no por factura»; «empresa emisora» no aparece en ese spec: es el texto de M-22 y contradice a ambos). §15 #5 |
| M-22-bis | El contacto obtiene si la factura es verídica, si los bienes/servicios se prestaron según lo pactado y confirma la fecha de pago (cláusula del motor b del modelo, agregada en esta versión) | implementado | Por factura, en `repoVerifTel` (l.24163); no versionado, clave estable como M-25. `DrawerVerificacion` (l.7215) lleva el checklist `{existencia, recepcion, fechaPago}` (l.7218; rótulos «Existencia de la factura» / «Recepción conforme» / «Fecha de pago», l.7312–7316; mesa `CHECKS` l.11555) y exige los tres checks más la fecha comprometida antes de confirmar (`completo`, l.7229); `verificarDeudor` (l.50248–50258) y `confirmarLlamadaTel` de `VerificacionTab` (l.11505–11513) persisten `{por, fecha, checklist, contacto, compromiso, respaldo, sinRespaldo, notas}`; `verifFactura` lo lee (`checks: [existencia, recepcion, fechaPago]`, l.3377–3383); regla 6 («checklist existencia/recepción/fecha», `reglas/verificacion.md` l.25). El veredicto congelado sigue siendo por deudor (`VERIF_VEREDICTO` l.24479). Pendiente sólo documental: `spec-verificacion-facturas.md` no describe el checklist (§2.1: el detalle por factura «no es responsabilidad de esta función») y el comentario «guarda `{por, fecha}` y nada más» de l.3367 quedó obsoleto (el comentario l.7189–7191 lo explica como el estado ANTERIOR) |
| M-23 | Si el deudor no pasa, todas las facturas de la oferta se verifican | decisión abierta | `verifFactura` l.3354 (todas las del deudor); «Regla 0» de verificación (regla 6; `spec-verificacion-facturas.md` §4.0: todas las de la oferta sólo para cliente nuevo) |
| M-24 | La verificación corre en cada evaluación | implementado distinto | Corre en el **render del detalle** (`verifFactura` l.3354) y en la versión (`snapVersionCli.verificacion` l.23157, sólo al hacer «Re-evaluación de la simulación»); ningún evento de evaluación la invoca: `simularOferta` (l.50148–50186) no llama a `verifDecision` y «Re-evaluar operación» (`reevaluarLinea` l.12456–12462) sólo apaga `reevalPend` tras 700 ms; regla 14 (gate 124); tras el contacto se congela |
| M-25 | Factura verificada no se vuelve a pedir; versionadas | implementado distinto | `repoVerifTel` l.24163 por (deal, factura); `congelarVeredicto` l.50387; `limpiarSimulacion` l.50187 no los toca; clave estable, no versión |
| M-26 | Líneas en cada evaluación, cada factura con línea asignada | implementado distinto | `asignarLineas` l.41762 (`CON_LINEA` / `REQUIERE_COMITE` l.41913 / l.41932) corre en el **render del detalle** (`evalLin` l.14349) y en la versión (`snapVersionCli` l.23244, sólo desde `reevaluarCliente` l.23373–23375 y `retirarFacturaOferta` l.50423); la simulación no lo ejecuta (`simularOferta` l.50148–50186 sólo escribe `simulado`, `stage` y `finanzasDe`) ni «Re-evaluar operación» (`reevaluarLinea` l.12456–12462); LIN-01 en `controlesIntegracion` l.23685 |
| M-27 | Cuatro líneas LF1–LF4 | implementado distinto | El modelo dice cuatro; el sistema asigna contra **cinco**: LF1–LF4 (cascada por estado del cliente, `spec-asignacion-lineas.md` §3.5; `asignarLineas` l.41835–41860) más la línea **global** del deudor como tercer nivel obligatorio —una factura cursa sólo si `m ≤ restPar && m ≤ dispCliente && m ≤ restDeudor` (l.41888); `capacidadDeudores` l.41719 «Nivel 3: la línea GLOBAL del deudor»— (regla 7 «cinco líneas»; origen A23 bloque `LINEA_DEUDOR`, regla 44; y la línea del RUT cliente ES la suma de sus líneas, regla 45, ADR-0011). El modelo debe decir si la línea global del deudor es parte del modelo de líneas o un control aparte (§16) |
| M-28 | Al publicar, solicitud automática al comité | implementado (a confirmar) | `cerrarOferta` l.47286 (llamada l.47421) → `solicitudComiteDeOferta` l.41387 (siempre `tipoLinea: "puntual"`, `propGlobal: 0`, l.41394–41411; el motivo sólo rotula) → `api1Inyeccion` l.42185; regla 15-bis. Estado **bajo la lectura «no existir», a confirmar por el usuario** (§9); si el modelo quiso decir literalmente «existiendo suficiente línea», el sistema NO lo hace —`solicitudComiteDeOferta` devuelve `null` salvo `ev.requiereComite > 0` (l.41387–41389) y `cerrarOferta` sólo inyecta si hay solicitud (l.47421–47441)— y la cláusula pasa a «implementado distinto» |
| M-29 | Comité no aprueba → retirar facturas sin línea y re-evaluar | pendiente | `api3EstadoProceso` l.42277: sólo «Aprobada» / «Observada»; sin manejador de rechazo; choca con reglas 13 y 33 |
| M-30 | La línea es un monto; la asignación es por factura | implementado | `asignarLineas` `resFacturas` con origen l.41913; regla núcleo 9; `spec-asignacion-lineas.md` §2.1, §2.4 |
| M-31 | Una factura puede asociarse a más de una línea (LF2–LF3) | implementado | `origen.push` l.41898; «una factura se reparte entre ambas» l.41944; sólo dentro de la cascada del par |
| M-32 | No se cursa con línea por monto parcial | implementado | l.41888 `m ≤ restPar` y ≤ `dispCliente` y ≤ `restDeudor`; si no, completa a `REQUIERE_COMITE` l.41932; LIN-01 (regla 41) |
| M-33 | Giro GE / GN según cliente nuevo, otorgamiento, verificación y líneas | implementado distinto | `asignarGiros` l.5471: líneas no entran al criterio; dos tipos; GN disyunción (`spec-modelo-giro.md` §2) |
| M-34 | Pricing asigna tasa por deudor | implementado | `spreadSugerido` l.2145; `SPREAD_MIN_DEUDOR` l.2078; `tasaDe` l.13502; `prorratearOperacion` l.5322 |
| M-35 | Tasa equivalente vs. tasa de los últimos negocios | implementado distinto | `tasaEquivalente` l.5276; `tasaUltimoNegocio` l.6762 (**el** último negocio del **cliente**, sintético y sembrado sólo por cliente en `historialComercial` l.6736, no del par); `tasaModo` l.4653 |
| M-36 | Condiciones comerciales versionadas | implementado distinto | `snapVersionCli` l.23157 y `huellaOperacion` l.24201 no contienen tasa, comisión ni anticipo; la simulación inicial no emite versión y la v1 se emite retroactiva en la primera re-evaluación (l.23373) |
| M-37 | Resultado: requisitos de excepción | implementado | `evaluarOtorgItems` l.22328; tab Otorgamiento y mesa; regla 54 |
| M-38 | Resultado: requisitos de verificación | implementado | `verifResumenDeal` l.3430; tab y mesa `VerificacionView` (regla 53) |
| M-39 | Resultado: asignación de líneas y solicitudes | implementado | `asignarLineas` devuelve `solicitudes`; `lineaDeVersion` l.41667; `solicitudComiteDeOferta` l.41387 |
| M-40 | Resultado: montos a girar GE / GN | implementado | `asignarGiros` `porTipo` / `porDeudor`; `giroDeal` / `giroCongelado` l.24447; regla 22 |
...
|---|---|---|
| Inbound | Hora de corte y de reinicio (M-07, M-08) y frecuencia de la corrida (M-02) como parámetros del tenant que el job del backend consuma (`spec-inbound-facturas.md` §10.4); hoy la ventana y la frecuencia existen como parámetros huérfanos | `23:00`, `06:00`, `hora de corte`, `HORA_CORTE`, `corte diario`: nada; `horaInicio` / `horaFin`: cuatro resultados, todos de configuración (`CFG_OPER_BASE` l.4635–4636 y los `<input type="time">` l.34098, l.34102) y ningún consumidor; `frecuenciaMin: 60` (l.4639): editable en Configuración › Operación (l.34110) y sin consumidor (l.1054 es un comentario): el cron es `setInterval(…, CRON_MS)` (l.49375–49377, sólo con `modoDemo !== false`, l.49374) y el cierre es por conteo (l.49527); la ventana y la frecuencia configuradas son decorativas |
| Inbound | Criterios por fecha de emisión, cesión previa y lista de emisores con tags (M-10): qué tags, quién la mantiene, si es un activo nuevo del cedente | No existen en `CRITERIO_PRED` l.4361; reclamo y NC sólo dentro de «Buena factura» (l.4368), no seleccionables por sí solos; los tags son del deudor (A3/A4) |
| Inbound | «Aceptaciones» de DTESync (M-01): qué es y si participa del filtro | No existe como campo ni regla en ningún activo ni spec (el `EstadoDTE` del A1 trae sólo `Reclamado`, `NotaCredito`, `FolioNotaCredito`; `facturaDeDTE` l.3949–3971); en el fuente sólo como dato sintético sin consumidor: `facturasDeCandidata` (l.30290) sortea `estado: "Aceptada" \| "Reclamada" \| "Sin acuse"` (l.30312, única aparición), que habría que retirar o conectar al A1 si el modelo confirma que la aceptación (acuse de recibo, Ley 19.983) es criterio de candidatura |
| Inbound | El fuente anuncia una re-simulación automática al llegar facturas que no ocurre —en el comentario y en la bitácora, no en la pantalla del detalle—: es la re-evaluación automática que M-12 pide y la regla 14 prohíbe; corregir el comentario y los mensajes de bitácora, retirar el banner muerto, o que la decisión #1 de §15 lo contemple | La pantalla del detalle no anuncia nada: el banner «Se está recalculando la simulación con el paquete actualizado» (l.13252–13262, texto l.13260) es inalcanzable por `deal.actualizando && !fullPage` (l.13252; también l.14252–14256), porque el único `<DealDrawer` pasa `fullPage` (l.50869–50872). Lo que promete la re-simulación es el comentario «el resultado re-simulado llega tras una latencia» (l.48862–48865), el estado `actualizando: true` (l.48874) y los `logSys` «Recalculando N oportunidad(es) … latencia estimada …» (l.48876) y «Recálculo aplicado» (l.48903); `aplicar` l.48886–48899 sólo anexa a `facturasDisponibles` y apaga `actualizando`, sin `simulado`, sin `finanzasDe`, sin versión |
| Inbound | Definición única de «Prime» para segmentar (M-05): sólo listas, o listas + nota > 4,2 | Listas en `CRITERIO_PRED` l.4376 y `capacidadDeudores` l.41700; disyunción con la nota en `verifDecision` (regla 6) |
...
| Otorgamiento | Excepción que deja de ser necesaria (M-21): eliminar visado, solicitud, tarea e hilo, o marcarlos «ya no aplica» | `huérfan`: sólo en Configuración › Áreas (l.35724 y l.35862, criterios que rutean a un área borrada), nada sobre visados; `ya no levanta`, `ya no aplica`, `visado obsoleto`: nada; `spec-gestion-excepciones.md` §5.5 no lo define |
| Otorgamiento | Visados «versionados» (M-20): dentro de cada versión o clave estable por criterio | `repoVisado` l.24142 (`VISADO_STATE` l.24462) por `stKey`; `snapVersionCli` l.23157 no lo contiene |
| Otorgamiento | Compuerta «sin excepciones sin justificar» en la mutación (M-19): hoy sólo en `ModalCurse`; por el principio de la regla 24, que el cierre ya aplica al monto, falta la guarda en `cerrarOferta` | `cerrarOferta` l.47286–47296 re-comprueba sólo `giroCursable`; entre l.47300 y l.47400 no consulta `excPend`, `sinComentario` ni el visado; el botón de `ModalCurse` `disabled={sinComentario > 0 \|\| !gOk}` (l.12293) y `if (!datosCurse) return;` (l.12745) son de pantalla; la Pre-evaluación solicita sin justificar (l.13193) |
| Otorgamiento | Tres gestos de evaluación: si el modelo quiere uno solo que siempre emita versión | `reevaluarCliente` l.23369 único emisor con variables frescas; `spec-gestion-excepciones.md` §4.1 |
| Otorgamiento | El camino vivo a giro salta Pendiente Integración, VER-01 y Operaciones N3 (desfase del sistema) | `useEffect` l.50629 escribe Girada; `avanzarPipeline` l.48913 sin llamador; `spec-ciclo-factura.md` §23c fila 4 |
...
| Verificación | Verificaciones «versionadas» (M-25): mismo dilema que los visados | `repoVerifTel` l.24163 por factura y `repoVerifVeredicto` l.24174 por deudor, fuera de la versión |
...
| Verificación | Qué obtiene el contacto (M-22-bis): **implementado**; pendiente sólo que `spec-verificacion-facturas.md` documente el checklist y corregir el comentario obsoleto de l.3367 («guarda `{por, fecha}` y nada más») | `DrawerVerificacion` l.7215: checklist `{existencia, recepcion, fechaPago}` (l.7218), rótulos l.7312–7316, `completo = chk.existencia && chk.recepcion && chk.fechaPago && !!compromiso` (l.7229), devuelve `checklist: chk` (l.7503); `verificarDeudor` l.50248–50258 y `VerificacionTab` l.11505–11513 persisten `{por, fecha, checklist, contacto, compromiso, respaldo, sinRespaldo, notas}` en `repoVerifTel`; `verifFactura` l.3377–3383 lee `checks`; mesa `CHECKS` l.11555; `VERIF_VEREDICTO` l.24479 sigue por deudor; la fecha de pago además como criterio previo (`spec-verificacion-facturas.md` §5 fila 6) |
| Líneas | Comité que no aprueba (M-29): contradicción 4 | `Rechazada por el comité`, `rechazarSolicitud`, `resolverComite`, `decisionComite`: nada; `api3EstadoProceso` l.42277 sólo Aprobada / Observada |
...
| Líneas | Si el tipo de la solicitud debe seguir al motivo (M-28): hoy la solicitud automática pide siempre líneas puntuales cliente-deudor aunque el motivo sea `cliente`, `deudor` o `lf1` | `solicitudComiteDeOferta` l.41394–41411 (`tipo: "modificar"`, `subtipo: "agregar_deudores"`, `propGlobal: 0`, `tipoLinea: "puntual"`); `RESOLUCION_COMITE` l.41366–41373 sólo aporta `pide` / `alcance`; `api1Inyeccion` l.42185 no transforma |
| Líneas | Si la línea global del deudor (compartida por todos los clientes que le ceden) forma parte del modelo de líneas (M-27 dice cuatro) o es un control aparte | `asignarLineas` l.41862–41868 (`lineaDeudorDe`) y l.41888 (`m ≤ restDeudor`); `capacidadDeudores` l.41719 «Nivel 3: la línea GLOBAL del deudor»; regla 7 (cinco líneas); regla 44 (A23 bloque `LINEA_DEUDOR`); regla 45 (la línea del RUT cliente ES la suma de sus líneas, ADR-0011) |
| Giro | El resultado de líneas no entra al criterio (M-33): que entre, o que siga sólo como LIN-01 | `asignarGiros` l.5471 recibe cuatro hechos, ninguno de líneas |
| Giro | Tercer tipo; GN disyunción o conjunción; pantalla de la asignación (§7 del spec vs regla 22) | `spec-modelo-giro.md` §2, §7 |
| Giro | Contrato de entrega a Tesorería (payload, endpoint, idempotencia, evento) | No modelado (`spec-ciclo-factura.md` §23b); `recibirGiroTesoreria` l.42253 es sólo el callback |
| Pricing | Tasa de referencia (M-35): el último negocio del cliente o una ventana de N negocios | «últimos negocios» (plural) o promedio: no existe; sólo `tasaUltimoNegocio` l.6762, del **cliente** (sintético, `historialComercial` l.6736 siembra sólo por cliente), no del par; `spec-pricing-simulacion.md` §4.2 fija **el** último negocio, en singular, como tasa top-down, y ningún spec define una ventana ni un promedio |
| Pricing | Condiciones comerciales en la versión (M-36); la simulación inicial no emite versión | `snapVersionCli` y `huellaOperacion` l.24201 no las contienen; `simularOferta` l.50148 no toca `repoSimVersions`; la v1 la emite retroactiva la primera re-evaluación (l.23373); `spec-ciclo-factura.md` §23c fila 5 |
| Pricing | El plazo por documento no llega al prorrateo (plazo por deudor) | Desfase medido, `spec-ciclo-factura.md` §23c fila 1 |
| Máquina de estados | No hay catálogo formal de transiciones: `spec-ciclo-factura.md` §20 dice que el resolver valida «contra la máquina de estados» sin definirla. La tabla de §2 es la primera enumeración, y es medida, no normativa | Transiciones repartidas entre `simularOferta`, `cerrarOferta`, `confirmarCierre` / `etapaTrasFirma`, `aprobarIntegracion`, `recibirGiroTesoreria`, `reabrirOperacion`, los dos escritores manuales `moverEtapa` (l.48330) y `moveTo` (l.48439) —con guardas parciales: `cesion` entra sin `integracion`, `perdida` sin causa, y volver a `oferta` / `prospeccion` no tiene guarda en `moverEtapa`— y los ocho escritores de pérdida |
| Máquina de estados | El id `aceptadas` sin escritor; si `Aceptada` y `Cesión` deben ser dos estados con la inscripción de la AEC entre medio (`spec-ciclo-factura.md` §23b) | `stage: "aceptadas"` como cadena literal: ningún escritor; la firma escribe `cesion` u `otorgamiento` (`patch` de `confirmarCierre` l.47771–47798) |
| Máquina de estados | `clienteAcepto` lo escribe el canal WhatsApp, no la firma: el intent `cursar` deja la operación en `oferta` con `clienteAcepto: true` y `aprobacionFormalCliente` la da por aceptada sin portal, contra la regla 1 | `grep clienteAcepto: true` → l.23607 (`dealFirmado`, base en memoria) y l.48080 (manejador `if (intent === "cursar" && hayOferta && !yaBoton)`, l.48046–48083); el `patch` de `confirmarCierre` (l.47771–47798) no lo escribe; `aprobacionFormalCliente` l.23590 lo lee |
