...
## Reglas de dominio (64 · todas con gate desde el 17-09-2026: 41 tenían caso en la suite —32 lo citan en su texto, 9 se les asignó por el título del caso—, la 32 tiene el suyo de contrato, y las 21 que iban sólo por revisión recibieron el suyo en la suite, en `tests/e2e/` o en `tests/contract/`)
...
| 2 | Nota Deudor 1–5 | [`reglas/clasificacion_deudor.md`](./reglas/clasificacion_deudor.md) | 117 |
...
| 5 | Pérdida es estado terminal | [`reglas/curse_firma_y_etapas.md`](./reglas/curse_firma_y_etapas.md) | 118, 119, `regla_5.test.mjs` |
...
| 8 | Oferta | [`reglas/oferta_pricing_y_giro.md`](./reglas/oferta_pricing_y_giro.md) | 120 |
...
| 10 | Contactabilidad | [`reglas/prospeccion_cartera_y_churn.md`](./reglas/prospeccion_cartera_y_churn.md) | 121, `regla_10.test.mjs` |
| 11 | Asignación de ejecutivo por CEDENTE | [`reglas/prospeccion_cartera_y_churn.md`](./reglas/prospeccion_cartera_y_churn.md) | 122 |
| 12 | La reserva NO es de NEX | [`reglas/lineas_y_solicitud_comite.md`](./reglas/lineas_y_solicitud_comite.md) | 123, `regla_12.test.mjs` |
...
| 13-terdecies | La LÍNEA GENERAL DEL CLIENTE va en la cabecera del detalle, y se mueve con la simulación | [`reglas/ui_detalle_y_tubo.md`](./reglas/ui_detalle_y_tubo.md) | 124, `e2e-13-terdecies`, `regla_13_terdecies.test.mjs` |
...
| 14 | Reevaluación explícita | [`reglas/oferta_pricing_y_giro.md`](./reglas/oferta_pricing_y_giro.md) | 125, `e2e-14-a`, `e2e-14-b`, `e2e-14-c`, `regla_14.test.mjs` |
| 15 | Solicitud de línea | [`reglas/lineas_y_solicitud_comite.md`](./reglas/lineas_y_solicitud_comite.md) | 126, `e2e-15`, `regla_15.test.mjs` |
...
| 15-bis-bis | La solicitud inyectada al cerrar la oferta tiene que CRUZAR de pestaña | [`reglas/lineas_y_solicitud_comite.md`](./reglas/lineas_y_solicitud_comite.md) | 127, `e2e-15-bis-bis-a`, `e2e-15-bis-bis-b`, `regla_15_bis_bis.test.mjs` |
...
| 15-quater-bis | El paso de DEUDORES del wizard se rediseñó | [`reglas/lineas_y_solicitud_comite.md`](./reglas/lineas_y_solicitud_comite.md) | 128, `e2e-15-quater-bis` |
...
| 27-bis | Los rótulos del menú: «Gestión diaria» y «Reportes» | [`reglas/ui_detalle_y_tubo.md`](./reglas/ui_detalle_y_tubo.md) | 129, `e2e-27-bis`, `regla_27_bis.test.mjs` |
...
| 34 | El lenguaje visual es Datamart: una referencia externa aporta composición, nunca tokens (y ningún `DESIGN.md` en la raíz) | [`reglas/ui_detalle_y_tubo.md`](./reglas/ui_detalle_y_tubo.md) | `regla_34.test.mjs` |
...
|---|---|---|---|---|
| TEN-01 | Aislamiento por tenant | Toda mutación usa el tenant fijado al abrir la sesión; si el tenant activo cambió sin re-autenticar, se rechaza. | servidor | 132 |
| RAT-01 | Límite de tasa por usuario | Máximo de mutaciones por minuto y por (usuario, familia de colecciones): una colección y su detalle (`CONTRATO_FAMILIA`) comparten el mismo tope. | servidor | 133 |
| IDM-01 | Idempotencia de la mutación | La misma mutación no se aplica dos veces. Acá sólo se CUENTAN los duplicados: la idempotencia real necesita una clave generada por el cliente, que este código todavía no emite. | servidor | 134 |
| LIN-01 | La operación no supera la línea disponible | El monto de la operación tiene que caber en la línea disponible del cliente al momento de armarla. | servidor | 135 |
| OTG-01 | Sólo aprueba quien tiene atribución | La excepción la resuelve un apoderado con atribución en el área y nivel que la regla exige. | servidor | 136, `regla_otg_01.test.mjs` |
...
| GIR-01 | No gira sin pasar por Cesión | El desembolso exige que la operación haya pasado por Cesión (documentos cedidos a Security). | servidor | 137 |
...
| ATR-01 | Descuento dentro de la atribución | El descuento aplicado no puede exceder la atribución del rol sin autorización de la jefatura correspondiente. | servidor | 138 |
| CRY-01 | El hash del OTP no sale del servidor | El OTP se guarda como SHA-256 con sal por emisión; el hash nunca viaja al cliente ni a otra página. La validación ocurre server-side, con límite de intentos y TTL. | servidor | 139, `e2e-CRY-01-limite`, `e2e-CRY-01-bajo-limite`, `regla_cry_01.test.mjs` |
| PRI-01 | La prioridad de curse la pide una jefatura | Marcar una oportunidad como prioritaria es una instrucción de jefatura, no del ejecutivo dueño del negocio. | servidor | 140 |
...
| `regla_<slug>.test.mjs` (19 archivos; 18 el 17-09-2026 y la 34 el 18-09-2026) | Un gate por regla de dominio que vive en JSX o en un closure y no tiene función pura que la suite pueda llamar: se fija sobre el TEXTO del fuente (el sitio, la compuerta, el rótulo, la unidad) y cada uno trae sus sondas | regla · alguno **snapshot** (`TOPE_REINTENTOS`) |
