...
- **Pasos**: 1) `aplicarActualizacionDTE` sobre la firmada con la NC y el reclamo → documentos marcados; con el acuse → sin marca; sobre la abierta → sin marca; 2) `verifResumenDeal` con y sin el veto → `pend` sube en uno con la llamada en verde; 3) `issueVerificacion` → título del SII, folio y motivo; mezclado con el veto de la llamada → título doble; sólo llamada → el de siempre; 4) `controlesIntegracion` → VER-01 nombra «inhabilitada(s) por el SII»; 5) `estadoCandidata` → «Inhabilitada en el SII» con la instrucción; 6) `avisarNoVerificadas` con el documento marcado → asunto «Documentos inhabilitados en el SII · OP», reusado; con uno de la llamada → «Verificación fallida · OP».
- **Resultado esperado**: el de cada paso. La pantalla (la fila «Inhabilitada en el SII», la tarjeta «· N en el SII», el hilo en el centro de mensajes) queda para un e2e cuando el stream sea determinista.
