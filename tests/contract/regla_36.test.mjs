/* Gate de contrato de la regla 36 (la Bandeja Inbound es una ventana con tope, y lo que el tope bota se
...
export function auditarRegla36(src) {
  const fallos = [];
  ...
  if (!/setBandejaRecortadas\(\(n\) => \(\{ total: n\.total \+ r\.fuera, conDueno: n\.conDueno \+ r\.fueraConDueno \}\)\)/.test(src)) fallos.push("el recorte de la bandeja no cuenta lo que saca: sin contarlo no hay cómo decirlo");
  if (!/logSys\("warn", "inbound", `La Bandeja Inbound llegó a su tope/.test(src)) fallos.push("el recorte no deja rastro en el log del sistema");
  if (!/const r = recortarBandeja\(junto, STREAM_TOPE\);/.test(src)) fallos.push("el stream ya no usa `recortarBandeja`: la política quedaría duplicada y podrían divergir");
  ...
  if (!/const inboundFilas = useMemo\(\(\) => \(showInbound \? agruparInboundPorCliente\(streamFeed, asignarEjecutivo\)\.length : 0\)/.test(src)) fallos.push("el contador de «Todos» no cuenta las filas agrupadas: contar facturas dice 262 sobre una tabla de 307");
  ...
  if (!/streamAgrupadoCliente = \(soloMias = true\) => agruparInboundPorCliente\(/.test(src)) fallos.push("la lista de la tabla ya no usa `agruparInboundPorCliente`: contador y lista podrían divergir");
  ...
  if (!/setStreamFeed\(\[\]\); setBandejaRecortadas\(\{ total: 0, conDueno: 0 \}\);/.test(src)) fallos.push("reiniciar la simulación no pone en cero lo recortado: arrastraría la cuenta de la corrida anterior");
  return fallos;
}

test("36 · la bandeja tiene tope con nombre honesto, alcanza para un lote, y lo que bota lo cuenta y lo dice", () => {
  assert.deepEqual(auditarRegla36(jsx), []);
});
...
const MUTANTES = {
  ...
  "el recorte deja de contar": { src: jsx.replace("          setBandejaRecortadas((n) => ({ total: n.total + r.fuera, conDueno: n.conDueno + r.fueraConDueno }));\n", ""), re: /no cuenta lo que saca/ },
  ...
  "el recorte deja de registrarse": { src: jsx.replace('logSys("warn", "inbound", `La Bandeja Inbound llegó a su tope', 'logSys("info", "inbound", `Otra cosa'), re: /no deja rastro en el log/ },
  ...
  "reiniciar arrastra la cuenta": { src: jsx.replace("setStreamFeed([]); setBandejaRecortadas({ total: 0, conDueno: 0 });", "setStreamFeed([]);"), re: /no pone en cero lo recortado/ },
};

for (const [nombre, m] of Object.entries(MUTANTES)) {
  test(`36 · sonda: el mutante «${nombre}» lo atrapa el auditor`, () => {
    assert.notEqual(m.src, jsx, "el mutante no cambió el fuente: la sonda no probaría nada");
    const fallos = auditarRegla36(m.src);
