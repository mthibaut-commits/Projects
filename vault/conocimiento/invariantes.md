...
| 63 | El lenguaje visual es Datamart: una referencia externa aporta composición, nunca tokens (y ningún `DESIGN.md` en la raíz); una skill de terceros que contradice una regla de acá se CORRIGE con su bloque de ajuste local | [`reglas/ui_detalle_y_tubo.md`](./reglas/ui_detalle_y_tubo.md) | `regla_63.test.mjs`, `skills.test.mjs` |
...
| `regla_<slug>.test.mjs` (43 archivos, 17-09-2026) | Un gate por regla de dominio que vive en JSX o en un closure y no tiene función pura que la suite pueda llamar: se fija sobre el TEXTO del fuente (el sitio, la compuerta, el rótulo, la unidad) y cada uno trae sus sondas | regla · alguno **snapshot** (`TOPE_REINTENTOS`) |
| `skills.test.mjs` (23-09-2026) | Las **skills de terceros** de `.claude/skills/`: qué skill contradice qué regla de este repo (**snapshot** `BASE_CHOQUES`, 12 de 39 medidas con `auditar_skills.mjs`) y que **cada una lleve su bloque `AJUSTE-LOCAL-NEX` antes del cuerpo**, citando la regla (**regla**, nunca se afloja: una reinstalación lo borraría en silencio, porque `skills-lock.json` guarda el hash de upstream) | **snapshot** · regla |
