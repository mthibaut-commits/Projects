...
- **Criterio**: CA-1 de HU-12 · **Dirección**: negativa · **Capa**: contrato + suite. · **Cobertura actual**: NUEVO → **decidido: implementar** (retirar el anuncio: G-09, GD-10, ADR-0013).
...
- **Criterio**: CA-3 de HU-12 · **Dirección**: negativa · **Capa**: contrato. · **Cobertura actual**: NUEVO.
...
- **Criterio**: CA-2 de HU-12 · **Dirección**: positiva (la dirección que no bloquea del control de CP-031) · **Capa**: suite. · **Cobertura actual**: NUEVO. El caso 124 fija que la re-evaluación emite versión; ningún caso lee la bitácora después de re-evaluar. D1 cerrada (ADR-0013): CP-032 cubría la lectura descartada y este es el caso de la elegida.
...
- **Criterio**: CA-1 de HU-21 · **Capa**: suite · **Cobertura actual**: NUEVO → **decidido: implementar** (ADR-0013 punto 5; G-22 y G-32; el caso 124 es el molde de la versión; los casos 16–19 el del diff).
...
- **Criterio**: CA-2 de HU-21 · **Dirección**: negativa · **Capa**: suite. · **Cobertura actual**: parcial — **caso 85** fija qué contiene la huella; no prueba que el precio no la mueva. **NUEVO** (junto al 85).
...
### CP-119 · Sobre la misma firmada, el retiro por `noConfirmada` SÍ pasa cuando lo dispara el EJECUTIVO: versión que sólo encoge, cursable que baja, folio vetado; la marca de la verificación por sí sola NO lo dispara
...
- **Precondición**: el mismo deal de CP-063 (`clienteAcepto: true`, `paqueteCerrado`), `n0 = (SIM_VERSIONS[id] || []).length`, una factura de la oferta marcada «no verificada» (veredicto en `repoVerifVeredicto` y la marca por factura, sin retiro).
...
### CP-091 · «No verificar» → «Retirar y vetar»: HOY sale de la oferta, `noConfirmada`, tachada, y el detalle exige re-evaluar (vigente hoy · cambia con ADR-0018: pasa a la dirección contraria)
...
### CP-129 · «No verificar» sobre la firmada HOY sólo encoge —la etapa no retrocede y la firma sigue— (vigente hoy · cambia con ADR-0018: pasa a la dirección contraria)
...
- **Criterio**: CA-1 de HU-42 · **Dirección**: negativa (el control bloquea el retiro: `facturasOp` y las versiones no cambian) y positiva (el issue existe y VER-01 lo nombra) · **Capa**: e2e + suite. · **Cobertura actual**: NUEVO → **decidido: implementar** (ADR-0018, G-36; T1). Nace en rojo: hoy `verificarDeudor`, `marcarFactura` y `noConfirmoDeudor` llaman a `retirarFacturaOferta` con `noConfirmada` al marcar (casos 21–23 fijan ese retiro y se re-anclan al ejecutivo, CP-119).
...
- **Criterio**: CA-5 de HU-42 · **Dirección**: negativa (no entra) y positiva (una factura del pool sin veto sí entra) · **Capa**: e2e + suite. · **Cobertura actual**: parcial — **caso 25** (la no confirmada queda vetada y no vuelve a entrar; regla 1) y **caso 95** (el `estadoCandidata` bloqueado) en la suite fijan el veto; en pantalla es la mitad de CP-091 que sobrevive al ADR y se mueve aquí: **NUEVO** en pantalla. El veto lo pone el retiro del ejecutivo (ADR-0018 punto 4), no la marca.
...
- **Criterio**: CA-1 de HU-42 (el segundo sitio: el detalle; ADR-0018 nombra «la mesa y el detalle») · **Dirección**: negativa (no retira) · **Capa**: e2e. · **Cobertura actual**: NUEVO → **decidido: implementar** (ADR-0018, G-36). Hoy el tab Verificación del detalle (`VerificacionTab`, visible desde la publicación, regla 58) ofrece «El deudor no confirmó · retirar» y su `ConfirmDialog` «¿El deudor no confirmó esta factura?» con el botón «Retirar factura no confirmada» llama al retiro con `noConfirmada`: nace en rojo.
