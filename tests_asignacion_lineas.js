...
(() => {
  ...
  // V04 aplica además al segmento PRIME, así que se llevaba también a los deudores de lista.
  {
    ...
    //     el layout viene en PESOS y sin sufijo de escala (regla 61), así que el valor leído tiene que
  ...
  //        mercado, sin nosotros (`candidatasDeCartera`, `CESIONARIOS_MERCADO`, `facturasDeCandidata`).
  {
    ...
    for (const ev of muestra) {
      ...
      // …descontando lo que el filtro de calidad excluye por el DOCUMENTO: cesión ajena (regla 64) y antigüedad (regla 65).
  ...
  // listándolas sólo desde ahí la evidencia de «esta no la confirmó» desaparecía de la pantalla.
  {
    ...
    if (!deudorTel157) { ok("157 la mesa trabaja por factura: cada documento trae su estado y su respaldo, y la retirada sigue en la mesa", false, "ningún deudor de prueba requiere verificación"); }
      ...
      const retiradaOk = !!fv && fv.docs.length === 3
        ...
        // y NO suma al monto: no está en la oferta
        && mm(fv.monto) === 50;
  ...
  // eso ya es trabajo de alguien. Se prueba en las DOS direcciones y en el borde —cerrada sin comunicar—,
  // porque publicar son dos hechos y con uno solo la casa todavía no se comprometió con el cliente.
  {
    ...
    const antesOk = exigeAcciones(null) === false
      ...
      && exigeAcciones({ stage: "prospeccion" }) === false;
    const despuesOk = exigeAcciones(publicada) === true && exigeAcciones(porNegocio) === true;
    // Y desde la firma en adelante se exige siempre, publicada o no: la operación ya no vuelve sola.
    const posteriores = ["aceptadas", "cesion", "otorgamiento", "giro"];
    const etapasOk = posteriores.every((st) => exigeAcciones({ stage: st }) === true)
      && ["prospeccion", "oferta", "perdida"].every((st) => exigeAcciones({ stage: st }) === false);
    // Es EXACTAMENTE `ofertaPublicada` antes de la firma: si divergieran, el tab de Verificación y su
    // badge dirían cosas distintas sobre la misma operación.
  ...
  // ═══ 159 · ADR-0014 / regla 64: la cedida a un factoring AJENO no es candidata del inbound; la cedida a Security sí ═══
  {
  ...
  {
    // 160 · La antigüedad máxima desde la emisión es condición de candidatura del inbound (regla 65) y parámetro del
  ...
  {
    // 161 · NINGUNA EXCEPCIÓN SIN JUSTIFICAR: la compuerta es de la MUTACIÓN (regla 66, M-19) y solicitar sin
    ...
    //       el gate de texto `regla_66.test.mjs` fija que la llama antes de escribir.
  ...
  {
    // 162 · EL QUINTO HECHO (ADR-0017, regla 67): si las facturas del deudor requieren comité, el giro es Normal aunque
  ...
  {
    // 163 · EL RELOJ DEL TENANT (ADR-0019, regla 68): el corte y el reinicio corren a la hora configurada —no por conteo
...

  {
    ...
    //       con id propio y referencia (ADR-0019, regla 68). CP-022, CP-023 y CP-143. El corte es una función pura sobre
  ...
  {
    // 165 · EL COMITÉ DE CRÉDITO QUE RECHAZA (ADR-0015, regla 69): la API 3 devuelve «Rechazada» por línea de detalle;
    ...
    //       CP-095, CP-096 y CP-133. La decisión es pura; el gate `regla_69.test.mjs` fija que «Consultar estados» la aplica.
    const nD = (r) => nomDe(r);
  ...
  {
    // 166 · LA EXCEPCIÓN QUE LA VERSIÓN N YA NO LEVANTA SE MARCA «ya no aplica desde la versión N», NO SE BORRA (regla 70,
  ...
  {
    // 167 · LA VERIFICACIÓN FALLIDA MARCA Y AVISA, NO RETIRA (regla 71, ADR-0018, M-18; G-11 en M-18 y G-36). «Si el
    ...
    //       tenga la excepción «noConfirmada» lo fija el gate de texto `regla_71.test.mjs` (son closures de React).
    const ID = "T-167";
  ...
  {
    // 168 · UN EVENTO DE EVALUACIÓN, CINCO MOTORES, CINCO VERSIONES CON EL MISMO NÚMERO (regla 72, ADR-0013; M-13, M-24,
    ...
    //           (regla 69), sin push (la decisión es pura);
    ...
    //       del recálculo se haya retirado lo fija el gate de texto `regla_72.test.mjs` (son closures de React).
    const ID = "T-168";
    const habia = repoSimVersions.get(ID);
    const auditSnap = AUDIT_LOG.slice(); const auditDesc = AUDIT_DESCARTADOS;
    const rateSnap = Object.fromEntries(Object.keys(RATE_BUCKETS).map((k) => [k, RATE_BUCKETS[k].slice()]));
  ...
  {
    // 169 · EL ACUSE DEL RECEPTOR ES UNA BANDERA DEL DTE QUE EL A1 TRAE Y `facturaDeDTE` LEE; SE MUESTRA Y NO FILTRA (regla 73,
  ...
  {
    // 170 · EL A1 ES UN FLUJO DE EVENTOS POR DOCUMENTO (regla 74, ADR-0020; el usuario, 23-09-2026: «los eventos de
    ...
    // Sobre la oferta cerrada la NC INHABILITA el documento (regla 75, ADR-0021): queda con su estado, marcado, con traza en rojo; y no dos veces.
...

  {
    ...
    //       CURSABLE (regla 75, ADR-0021; el usuario, 23-09-2026: «se debe dejar la oferta como no cursable, el documento
