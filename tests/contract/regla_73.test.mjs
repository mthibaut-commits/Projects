/* Gate de contrato de la regla 73 (ADR-0020: el A1 es un flujo de eventos por documento), sobre el TEXTO del fuente.
...
export function auditarRegla73(src) {
  ...
  // 6 · Sobre la oferta cerrada, la NC o el reclamo se aplican Y la inhabilitan (regla 74 fija el resto).
...
test("regla 73: el A1 es un flujo de eventos que se pliega en un solo sitio, con el mismo pliegue que el generador; el stream y el tick separan las actualizaciones; la oferta cerrada distingue la inhabilitación", () => {
...
/* Las sondas se plantan sobre el texto CRUDO (la función extraída lleva un comentario de línea, y `canonico` la
   rompería); `auditarRegla73` canoniza por dentro. */
const MUTANTES = [
