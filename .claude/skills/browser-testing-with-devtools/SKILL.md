...
>
> - **Capa unitaria, Jest/vitest, umbral de cobertura** → acá **no existe esa capa y es deliberado**: el fuente es UN archivo (`pipeline_comercial.jsx`) y las funciones puras (`asignarLineas`, `verifDecision`, `prorratearOperacion`) ya se prueban por su nombre inyectándoles el estado, desde la suite que corre en Chromium real. Las tres capas y por qué no hay una cuarta: `.claude/rules/testing.md`.
> 
> - **El ciclo rojo → verde → refactor SÍ vale** y es la disciplina de la casa (`.claude/rules/workflow.md`, paso 3): el caso en rojo va ANTES de la implementación. Lo que cambia es dónde vive el caso — la suite, un `e2e-<regla>` o un `regla_<slug>.test.mjs` —, no si se escribe primero.
