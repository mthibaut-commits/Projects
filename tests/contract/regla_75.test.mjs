/* Gate de contrato de la regla 75 (ADR-0021: la NC, el reclamo o la cesión a otro sobre una oferta cerrada, publicada o
   ...
   veto por el ÚNICO escritor (`marcarNoVerificada`, regla 71) y FUERA del updater, que ese escritor firme como el SII,
...
export function auditarRegla75(src) {
  const fallos = [];
  ...
  if (!tk) fallos.push("no encuentro `aplicarActualizacionesDTE`");
    ...
    if (iVeto < 0) fallos.push("el tick no escribe el veto del SII por `marcarNoVerificada` (regla 71: un solo escritor)");
  ...
  if ((can.match(/repoNoConfirmadas\.set\(/g) || []).length !== 1) fallos.push("el veto tiene más de un escritor (o ninguno): la regla 71 exige uno solo");
  ...
  if (!can.includes("inhabilitada(s) por el SII: reclamo, nota de crédito o cesión a otro, regla 75")) fallos.push("VER-01 no nombra las inhabilitadas por el SII");
...
test("regla 75: sobre la oferta cerrada, publicada o firmada la NC, el reclamo o la cesión a otro inhabilitan el documento; el veto lo escribe el SII por el único escritor, cuenta como pendiente aunque la llamada esté en verde, y el issue, VER-01, la candidata, el aviso, la tarjeta y la fila lo dicen", () => {
  assert.deepEqual(auditarRegla75(jsx), []);
});
...
const MUTANTES = [
  ...
  ["VER-01 calla", (c) => c.replace("inhabilitada(s) por el SII: reclamo, nota de crédito o cesión a otro, regla 75", "")],
...
for (const [nombre, mutar] of MUTANTES)
  test(`sonda negativa: ${nombre}`, () => {
    ...
    assert.ok(auditarRegla75(mut).length > 0, "el gate no cazó la violación plantada");
  });
