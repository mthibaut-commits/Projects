/* Gate de contrato de la regla 69 (ADR-0016: la excepción que la versión N ya no levanta se marca «ya no aplica desde
   la versión N», no se borra; la marcada no se reactiva), sobre el TEXTO del fuente. La decisión pura y el flujo entero
   los prueba el caso 166 (`excepcionesQueYaNoAplican`, `reevaluarCliente` —que desde la regla 71 pasa por
...
export function auditarRegla69(src) {
  const fallos = [];
  const can = canonico(src);
  // 1 · La re-evaluación marca DESPUÉS de emitir la versión, con el número de esa versión.
  // Desde la regla 71 (ADR-0013) la versión la emite el EVENTO, `evaluarOperacion`, y ahí mismo se marca.
...
test("regla 69: la excepción que la versión N ya no levanta se marca, no se borra; ningún lector trata la marca como decisión", () => {
  assert.deepEqual(auditarRegla69(jsx), []);
});
...
for (const [nombre, mutar] of MUTANTES)
  test(`sonda negativa: ${nombre}`, () => {
    ...
    assert.ok(auditarRegla69(mut).length > 0, "el gate no cazó la violación plantada");
  });
