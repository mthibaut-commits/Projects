...
export function documentoEnPesos(src0) {
  ...
  const fila = entre(src, "{disp ? (sinDisp ? (<span style={{color: C.faint}}>—</span>)", "· tasa {tasa}%");
  if (!fila) fallos.push("no encuentro el bloque del monto del deudor (de «—» a «· tasa {tasa}%»)");
    ...
    if (!/<>\s*\{fmtCLP\(ev\.asignado\)\}\s*<span[^>]*>[\s\S]{0,80}?con línea, de \{fmtCLP\(monto\)\}\s*<\/span>\s*<\/>/.test(fila)) fallos.push("el monto del deudor («$X con línea, de $Y») no va en fmtCLP");
...
export function resumenesEnMM(src0) {
  ...
  const ind = entre(src, "function IndicadorLinea(", "} function ");
  if (!ind) fallos.push("no encuentro IndicadorLinea");
  else { if (/fmtCLP\(/.test(ind)) fallos.push("IndicadorLinea formatea en pesos"); if (!/\{fmtMM\(L\.aprobada\)\}(\{" "\})?\s*<span[^>]*>\s*aprobada\s*<\/span>/.test(ind) || !/`Disponible \$\{fmtMM\(L\.disponible\)\}`/.test(ind)) fallos.push("IndicadorLinea no dice «M$X aprobada» / «Disponible M$X»"); }
  const giro = entre(src, "function ChipGiro(", "} function ");
...
test("29 · SONDAS: cada violación plantada en una copia del fuente hace fallar a su detector", () => {
  ...
  const sondas = [
    ["titular en M$", veredictoEnPesos, can.replace(b, b.replace(/fmtCLP\(/g, "fmtMM("))],
    ["Total oferta en M$", documentoEnPesos, can.replace("<span>{fmtCLP(totalOf)}</span>", "<span>{fmtMM(totalOf)}</span>")],
    ["Línea disponible en M$", documentoEnPesos, can.replace("const lbl = hay ? `Línea disponible ${fmtCLP(ld.disponible)}`", "const lbl = hay ? `Línea disponible ${fmtMM(ld.disponible)}`")],
    ["tooltip de Línea disponible en M$", documentoEnPesos, can.replace("`Línea disponible de este deudor: ${fmtCLP(ld.disponible)}", "`Línea disponible de este deudor: ${fmtMM(ld.disponible)}")],
    ["Solicitud línea en M$", documentoEnPesos, can.replace("`Solicitud línea ${fmtCLP(solicitud)}`", "`Solicitud línea ${fmtMM(solicitud)}`")],
    ["tooltip de Solicitud línea en M$", documentoEnPesos, can.replace("const tipSol = `Sin cupo para ${fmtCLP(solicitud)}", "const tipSol = `Sin cupo para ${fmtMM(solicitud)}")],
    ["Solicitud línea fuera de la oferta", documentoEnPesos, can.replace("const solicitud = !enOferta ? 0 : ev ? faltante : hay ? 0 : monto;", "const solicitud = ev ? faltante : hay ? 0 : monto;")],
    ["Solicitud línea por el deudor entero y no por el faltante", documentoEnPesos, can.replace("const faltante = enOferta && ev ? Math.max(0, mmRound(monto - ev.asignado)) : 0;", "const faltante = enOferta && ev ? monto : 0;")],
    ["el chip se dibuja sin plata que pedir", documentoEnPesos, can.replace('{solicitud > 0 && (<ChipFila fg="#C2410C"', '{true && (<ChipFila fg="#C2410C"')],
    ["Solicitud línea en gris", documentoEnPesos, can.replace('<ChipFila fg="#C2410C" bg="#FFF7ED" borde="#F97316"', '<ChipFila fg="#6B7280" bg="#FFF7ED" borde="#F97316"')],
    ["Solicitud línea con fondo gris", documentoEnPesos, can.replace('<ChipFila fg="#C2410C" bg="#FFF7ED" borde="#F97316"', '<ChipFila fg="#C2410C" bg="#F3F4F6" borde="#F97316"')],
    ["monto del deudor «con línea, de» en M$", documentoEnPesos, can.replace("<> {fmtCLP(ev.asignado)} <span", "<> {fmtMM(ev.asignado)} <span")],
    ["monto del deudor, rama simple, en M$", documentoEnPesos, can.replace("</span> </>) : (fmtCLP(monto))}", "</span> </>) : (fmtMM(monto))}")],
    ["monto del deudor fuera de la oferta en M$", documentoEnPesos, can.replace("—</span>) : (fmtCLP(disp.monto))", "—</span>) : (fmtMM(disp.monto))")],
    ["chip de sección en pesos", resumenesEnMM, can.replace('title="Monto seleccionado para esta oferta" > {fmtMM(totalOf)}', 'title="Monto seleccionado para esta oferta" > {fmtCLP(totalOf)}')],
    ["indicador de línea en pesos", resumenesEnMM, can.replace('{fmtMM(L.aprobada)}{" "}', '{fmtCLP(L.aprobada)}{" "}')],
    ["badge de Giro en pesos", resumenesEnMM, can.replace("badge={soloTipo ? null : fmtMM(monto || 0)}", "badge={soloTipo ? null : fmtCLP(monto || 0)}")],
    ["CTA sin comité", ctaComite, can.replace('k === "cerrar" && aComite ? "Enviar a Comité y Publicar"', 'k === "cerrar" && aComite ? "Cerrar oferta y publicar"')],
    ["CTA sin mirar requiereComite", ctaComite, can.replace("{labelAccion(accionSel, !!evalLin && evalLin.requiereComite > 0)}", "{labelAccion(accionSel, false)}")],
  ];
  const mudas = sondas.filter(([, det, src]) => src === can || det(src).length === 0).map(([n]) => n);
