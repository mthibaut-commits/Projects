...
>
> - **Conventional Commits (`feat:`, `fix:`) en inglés** → acá el mensaje va **en español (Chile)**, descriptivo, y dice **qué y por qué** con el estilo del `git log` de la casa (reglas núcleo 5 y 6). Sin prefijo de tipo.
> 
> - **El flujo de ramas lo decide `flujo_git.md` y lo hace cumplir el hook `gitflow_guard`**: `main` estable, se trabaja en la rama designada de la sesión o en `feature/<slug>`, y la integración es `merge --no-ff` — nunca squash, nunca un push por refspec sobre `main`.
